...
function addElectronOutput(state, streamName, chunk) {
  ...
  if (process.env.DEBUG_DCODE_E2E === "1") process.stderr.write(text);
}
...
function startElectron(state) {
  const child = spawn(
    ...
    ],
    {
      ...
      env: {
        ...process.env,
        DCODE_DATA_DIR: state.dataDir,
        DCODE_HOST_BIN: state.hostBinary,
        DCODE_PLAN_UI_PROBE: "1",
  ...
  await waitFor(
    async () => {
      const available = await state.mainCdp.evaluateMain(
        "typeof globalThis.__DCODE_PLAN_UI_PROBE === 'function'",
        "Electron Main",
      );
      ...
      try {
        ...
        return targets.find(
          (target) =>
            ...
            (target.url.startsWith("file:") || target.title === "dcode"),
        );
      } catch {
        return null;
      }
...
async function selectSession(state, sessionId) {
  const selection = await state.cdp.evaluate(`(async () => {
    ...
    if (typeof window.__DCODE__?.selectSession === "function") {
      await window.__DCODE__.selectSession(wanted);
...
async function runPlanProbe(state, request) {
  ...
  const response = await state.mainCdp.evaluateMain(
    `(async () => {
      const probe = globalThis.__DCODE_PLAN_UI_PROBE;
...
async function verifyArtifact(state, proposal, checkpoint) {
  ...
  assert(
    /^\.(?:dcode|pi)\/plan\/[^/\\]+\.md$/.test(artifact.relativePath),
...
async function runAcceptance(state) {
  ...
  if (!LIVE_ENV_AVAILABLE) {
    console.log(
      `SKIP ${LIVE_CASE_ID} - set DCODE_TEST_API_KEY, DCODE_TEST_BASE_URL, and DCODE_TEST_MODEL for the optional live acceptance case`,
    );
  } else {
    try {
...
async function main() {
  ...
  const callerArtifactDir = process.env.DCODE_E2E_ARTIFACT_DIR?.trim();
