#!/usr/bin/env node
/**
 * Composer paste regression with the real Vue composables, real preload, and the
 * real scratch writer.
 *
 ...
 * devDependency of this repo (`esbuild` only arrives transitively); the bundler
 * and its Vue plugin are located through `createRequire`, so no path is
 * hardcoded. The scratch writer is still bundled with `esbuild` as a CJS
 * module — it is plain TypeScript, not an SFC.
 *
...
// this runner resolves (same line as `scripts/e2e-rpc-unicode.mjs`).
...
try {
  await build({
    ...
    build: {
      ...
      write: true,
      lib: {
        entry: join(root, "scripts/e2e/composer-paste.ts"),
        formats: ["iife"],
  ...
  await esbuild({
    entryPoints: [join(root, "src/main/composer-paste.ts")],
  ...
  await writeFile(
    ...
    await readFile(join(root, "src/renderer/styles/composer.css")),
  );
  await writeFile(
    join(temp, "index.html"),
    `<!doctype html><meta charset="utf-8"><meta http-equiv="Content-Security-Policy" content="default-src 'self'; style-src 'self' 'unsafe-inline'"><link rel="stylesheet" href="composer.css"><body><input id="native-files" type="file" multiple hidden><script src="bundle/renderer.js"></script>`,
  );
  await writeFile(
    join(temp, "main.cjs"),
    `
...
const saved = [];
const history = [];
ipcMain.handle("dcode/composer/pasteFiles", async (_event, input) => {
...
ipcMain.handle("dcode/clipboard/recordPaste", (_event, input) => {
