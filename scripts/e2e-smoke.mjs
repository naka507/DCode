#!/usr/bin/env node
/**
 * dcode e2e smoke tests (headless protocol-level).
 * Covers host-core RPC, tools, secrets, plugins, and optional live model chat.
 *
 * Env:
 *  DCODE_TEST_BASE_URL (required; there is no default)
 *  DCODE_TEST_MODEL
 *  DCODE_TEST_API_KEY
 *  DCODE_HOST_BIN (optional)
 */
...
const configuredHostBin = process.env.DCODE_HOST_BIN?.trim();
...
const hostBinaryName = `DCore${process.platform === "win32" ? ".exe" : ""}`;
...
// No default: a missing value used to point the whole suite (including the
// fixture provider) at a third-party gateway. Refuse to run instead.
const BASE_URL = process.env.DCODE_TEST_BASE_URL?.trim();
if (!BASE_URL) {
  console.error(
    "DCODE_TEST_BASE_URL is required (no default). Set it to the " +
      "OpenAI-compatible base URL this suite may talk to, for example " +
      "https://your-gateway.example/v1",
  );
  process.exit(1);
}
const MODEL = process.env.DCODE_TEST_MODEL || "mimo-v2.5";
...
const dataDir = mkdtempSync(join(tmpdir(), "dcode-e2e-"));
const results = [];
...
class Host {
  constructor(bin, dataDir) {
    ...
    this.child = spawn(bin, [], {
      ...
      env: { ...process.env, DCODE_DATA_DIR: dataDir },
    });
...
async function main() {
  ...
  try {
    ...
    if (API_KEY) {
      ...
      skip("E2E-008-live-model", "DCODE_TEST_API_KEY not set");
      skip("E2E-009-stream", "DCODE_TEST_API_KEY not set");
    }

    // agent-runtime unit-ish import check
    try {
      await import(pathToFileURL(join(root, "src/agent/runtime/index.ts")).href);
