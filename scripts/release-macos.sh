...
if [[ -z "${MAC_SIGNING_IDENTITY:-}" ]]; then
  ...
  echo "For an unsigned local build use: pnpm --filter @dcode/desktop dist" >&2
  exit 1
fi
...
pnpm -r --filter '!@dcode/desktop' build

echo "==> Building + packaging desktop (signed, $MAC_ARCH)"
pnpm --filter @dcode/desktop exec electron-vite build
pnpm --filter @dcode/desktop exec electron-builder --mac "--${MAC_ARCH}" \
  -c.mac.identity="${MAC_SIGNING_IDENTITY}" \
  -c.mac.forceCodeSigning=true \
  -c.mac.notarize=true

scripts/staple-macos-release-dmg.sh release
scripts/verify-macos-release.sh release
