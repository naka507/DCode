...
const previousBinary = process.env.DCODE_HOST_BIN;
let host;
let sidecar;
let server;
const requests = [];
...
try {
  ...
  await build({
    stdin: {
      contents: 'export { HostProcess } from "./src/main/host-process"; export { AgentSidecar } from "./src/main/agent-sidecar";',
      resolveDir: root,
      loader: "ts",
    },
    outfile: bundle,
  ...
  process.env.DCODE_HOST_BIN ??= join(root, "target/debug", `DCore${process.platform === "win32" ? ".exe" : ""}`);
  ...
  if (previousBinary === undefined) delete process.env.DCODE_HOST_BIN;
  else process.env.DCODE_HOST_BIN = previousBinary;
