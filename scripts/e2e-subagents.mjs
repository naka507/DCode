/**
 * dcode subagent registry e2e (headless protocol-level, D202).
...
// The agent-runtime sources use bundler-style `.js` specifiers and
// `@dcode/*` aliases, so the shared test hooks are required to import the
// real loader instead of a compiled barrel.
register(new URL("../tests/helpers/ts-import-hooks.mjs", import.meta.url));
...
try {
  ...
  const { loadSubagentDefinitions } = await import(
    join(root, "src", "agent-runtime", "subagent-definitions.ts")
  );
