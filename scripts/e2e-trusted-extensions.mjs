 ...
 * Override DCODE_HOST_BIN to select a specific host binary.
 */
...
const harness = join(root, "tests", "e2e", "trusted-extensions");
...
function findHostBinary() {
  const name = `DCore${process.platform === "win32" ? ".exe" : ""}`;
  return candidatesFor(process.env.DCODE_HOST_BIN?.trim(), [name]).find(existsSync);
}
...
async function main() {
  ...
  if (!hostBin) {
    throw new Error(`host binary missing; tried ${candidatesFor(process.env.DCODE_HOST_BIN?.trim(), [`DCore${process.platform === "win32" ? ".exe" : ""}`]).join(", ")}`);
  }
  ...
  const electron = spawnChild(
    ...
    ["."],
    {
      cwd: root,
      env: isolatedEnv({
        DCODE_HOST_BIN: hostBin,
        DCODE_MCP_CONTROL: "1",
        DCODE_MCP_PORT: String(mcpPort),
