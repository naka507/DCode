...
const { resolveSessionMessageInput } = await import("../src/engine/session-message-input.ts");
const { formatSessionMessage } = await import("../src/shared/session-collaboration.ts");
...
try {
  ...
  child = spawn(process.execPath, [fileURLToPath(new URL("../resources/runtime/sidecar.js", import.meta.url))], { stdio: ["pipe", "pipe", "pipe"] });
