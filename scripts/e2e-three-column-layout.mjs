 ...
 *
 * Prereqs: `pnpm --filter @dcode/desktop build` (or `pnpm build:js`) and a
 * host-core binary (target/debug, target/release, or DCODE_HOST_BIN).
 */
...
const cdpPort = Number(process.env.DCODE_LAYOUT_CDP_PORT || 9336);
const MAIN_PANE_MIN_WIDTH = 450;
const MAIN_PANE_REOPEN_TARGET_WIDTH = 460;

function resolveHostBinary() {
  const candidates = [
    process.env.DCODE_HOST_BIN?.trim(),
    join(root, "target", "debug", "DCore"),
    join(root, "target", "debug", "DCore.exe"),
    join(root, "target", "release", "DCore"),
    join(root, "target", "release", "DCore.exe"),
  ].filter(Boolean);
  const found = candidates.find((candidate) => existsSync(candidate));
  if (!found) {
    throw new Error(
      `host-core binary not found; run cargo build -p host-core or set DCODE_HOST_BIN\nchecked: ${candidates.join(", ")}`,
    );
  }
  return found;
}
...
async function main() {
  if (!existsSync(join(appDir, "out/main/index.js"))) {
    console.error("desktop app not built. Run: pnpm --filter @dcode/desktop build");
    process.exit(1);
  }
  ...
  const child = spawn(
    ...
    [`--remote-debugging-port=${cdpPort}`, `--user-data-dir=${profileDir}`, "."],
    {
      ...
      env: {
        ...process.env,
        DCODE_DATA_DIR: dataDir,
        DCODE_HOST_BIN: hostBinary,
  ...
  try {
    ...
    await rig(`window.__DCODE__.ensureVisualFixtures()`);
    ...
    await rig(
      `window.__DCODE__.selectSession(${JSON.stringify(sidebarSeed.alphaSessionIds[0])})`,
    );
    await waitFor(
      () =>
        cdp.evaluate(
    ...
    await rig(`window.__DCODE__.openWorkPanel()`);
    await rig(`window.__DCODE__.setWorkPanelWidth(720)`);
    ...
    await rig(`window.__DCODE__.collapseWorkPanel()`);
    await delay(700);
    if ((await measure()).sidebarKind !== "sidebar") {
      await clickSidebarToggle();
    }
    await rig(`window.__DCODE__.setWorkPanelWidth(244)`);
    await rig(`window.__DCODE__.openWorkPanel()`);
    ...
    await rig(`window.__DCODE__.setWorkPanelWidth(500)`);
    await waitFor(
    ...
    const previewActions = await cdp.evaluate(`(() => {
      ...
      return {
        platform: window.dcode?.platform ?? "unknown",
    ...
    for (const route of ["plugins", "pulls", "scheduled"]) {
      if ((await measure()).sidebarKind !== "sidebar") await clickSidebarToggle();
      await rig(`window.__DCODE__.setPage(${JSON.stringify(route)})`);
      await waitFor(
      ...
      for (const theme of ["light", "dark"]) {
        await cdp.evaluate(`window.__DCODE__.setThemeAttr(${JSON.stringify(theme)})`);
    ...
    check(
      ...
      "the group's own tail leaves with its rows: the section is its header plus the 1px scroller gap, and the rows sit past the clipped edge",
    ...
    check(
      ...
      "the group leaves the tab order while its rows stay mounted",
    ...
    await waitFor(
      ...
      "the group reopens after the reduced-motion probe",
    );
    await cdp.evaluate(`document.documentElement.dataset.theme = ${JSON.stringify(originalTheme)}`);

    await checkSidebarRowStates({ cdp, check, waitFor, seed: sidebarSeed });
    await checkSidebarSettings({
      cdp, check, waitFor, artifactDir: process.env.DCODE_LAYOUT_ARTIFACT_DIR,
    });
