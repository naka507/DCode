...
const limits = {
  ...
  appStore: {
    path: "src/renderer/stores/app-store.ts",
    max: 1_000,
  },
  newTypeScript: {
    extension: ".ts/.tsx",
...
/**
 * Every source file the gate measures: tracked, plus untracked-but-not-ignored.
 *
 * `git ls-files -c` reports *index* entries, not working-tree files, so a file
 * deleted in the working tree but not yet staged still appears here — and
 * `locFor` then dies on `ENOENT` instead of reporting a violation. Filtering on
 * `existsSync` measures what is actually on disk, which is what the ceiling is
 * about: a deleted file has no LOC to count.
 */
...
function parseArgs() {
  ...
  let base = process.env.ARCHITECTURE_BASE || defaultBranch();
...
/**
 * The repository's default branch, or null when no candidate resolves.
 *
 * The default branch here is `master` (AGENTS.md § 5). Hardcoding `main`
 * resolved nowhere, so the gate fell back to `HEAD^` — one commit deep, so a
 * new over-limit file added earlier on the branch passed the 800-LOC check
 * unnoticed. Asking for the default branch instead keeps the gate working if
 * the branch is renamed again.
 */
