...
import { PROTOCOL_VERSION as SHARED_PROTOCOL_VERSION } from "../src/shared/protocol.ts";
...
const LONG_TIMEOUT_ENABLED = process.env.DCODE_E2E_LONG_TIMEOUT === "1";
...
async function scenario115(binary, tempRoot) {
  return withScenario("E2E-115", async (ctx) => {
    ...
    if (!LONG_TIMEOUT_ENABLED) {
      skip(
        "E2E-115-60s-default",
        "DCODE_E2E_LONG_TIMEOUT=1 is not set; skipped the 60-second default leg",
      );
...
async function main() {
  ...
  const tempRoot = await mkdtemp(join(tmpdir(), "dcode-plan-e2e-"));
  try {
