...
} from "../src/renderer/lib/sidebar-preferences.ts";
import { loadRecentProjects, renameRecentProject } from "../src/renderer/lib/recent-projects.ts";
...
test("home switcher lists retained sidebar projects and the active workspace", () => {
  const projects = listSwitcherProjects({
    openProjectPaths: ["/Users/lan/dcode", "/Users/lan/dcode-plugins"],
    openProjects: [
      { path: "/Users/lan/dcode", name: "dcode" },
      { path: "/Users/lan/dcode-plugins", name: "dcode-plugins" },
    ],
...
test("home switcher search matches name or path and ignores case", () => {
  const projects = [
    { key: "/tmp/dcode", path: "/tmp/dcode", name: "dcode", pinned: false },
    {
      key: "/tmp/plugins",
      path: "/tmp/plugins",
      name: "dcode-plugins",
  ...
  assert.deepEqual(
    filterSwitcherProjects(projects, "/tmp/dcode").map((project) => project.name),
    ["dcode"],
  );
