 ...
 * pin.
 *
 * The module under test (`src/components/settings/subagent-models.ts`) is
 * framework-free and is a verbatim copy here, so the pure-function cases move
 * unchanged apart from the module path
 * (`src/components/settings/…` → `src/renderer/components/settings/…`) and the
 * import form (`test/helpers/ts-import-hooks.mjs` registers as
 * `tests/helpers/ts-import-hooks.mjs` here).
 *
 * The four source-contract cases were retranslated, not dropped:
 *
 *  - `SubagentEditorSheet.vue`. The model field is still sliced out between its
 *    two `Field` labels, but the labels are the template's single-quoted
 *    `t('…')` form and the free-text negative check reads the `<Input` tag.
 *  - `subagentModelChoices(providers)` → `subagentModelChoices(providers.value)`
 *    and `resetSubagentTemplate(draft)` → `resetSubagentTemplate(props.draft)`:
 *    a Vue SFC reads reactive props through `.value` / `props`, which is the
 *    same call with the same argument.
 *  - `SubagentModelPicker.vue`. `<AnchoredMenu`, the two `provider-service-*`
 *    class names, the search/no-match i18n keys and the group heading all
 *    survive; only the keyboard guard spelling differs, from
 *    `visibleIds.includes(activeId)` to
 *    `visibleIds.value.includes(activeId.value)`.
 *  - the `model-config.css` case is unchanged: the stylesheet keeps both rules
 *    and their declarations at `src/renderer/styles/model-config.css`.
 */
...
} = await import("../src/renderer/components/settings/subagent-models.ts");
...
test("the editor model field is a configured-only searchable picker with an empty-state action", async () => {
  const source = await readFile(
    new URL(
      "../src/renderer/components/settings/SubagentEditorSheet.vue",
      import.meta.url,
    ),
    "utf8",
  );
  const modelField = source.slice(
    source.indexOf("t('extensions.subagents.model')"),
    source.indexOf("t('extensions.subagents.thinking')"),
  );
  assert.ok(modelField.length > 0, "the model field must sit between its two labels");
  ...
  // The field's empty-state button calls a local `openModelSettings`
  // handler (the template has no inline store access); the handler is what
  // calls `setSettingsTab("agent")`, so both halves are pinned.
  assert.match(modelField, /@click="openModelSettings"/);
  assert.match(source, /function openModelSettings\(\): void \{[\s\S]*?setSettingsTab\("agent"\)/);
  ...
  assert.match(source, /subagentModelChoices\(providers\.value\)/);
  assert.match(source, /resetSubagentTemplate\(props\.draft\)/);
});

test("the model menu scrolls inside itself and filters, instead of using an OS-drawn list", async () => {
  const picker = await readFile(
    new URL(
      "../src/renderer/components/settings/SubagentModelPicker.vue",
      import.meta.url,
    ),
    "utf8",
  );
  ...
  assert.match(picker, /visibleIds\.value\.includes\(activeId\.value\)/);
});

test("the shared option-menu styles bound the list height and let it scroll", async () => {
  const styles = await readFile(
    new URL("../src/renderer/styles/model-config.css", import.meta.url),
    "utf8",
  );
...
test("the editor exposes the no-pass thinking option", async () => {
  ...
  // The slice runs from the thinking `Field` label to the end of the file, and
  // the Vue sheet builds the options array in a `computed` above the template,
  // so the slice is retranslated to the array itself: same three claims about
  // the same list.
  const start = source.indexOf("const thinkingOptions");
  assert.notEqual(start, -1, "the thinking options list is missing");
  const thinkingField = source.slice(start, source.indexOf("]);", start));
