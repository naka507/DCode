...
const [store, sidebarPreferences, api, main, protocol, runtime] =
  await Promise.all([
    readStoreSource(),
    read("../src/renderer/lib/sidebar-preferences.ts"),
    read("../src/renderer/lib/api.ts"),
    readMainSource(),
    read("../src/shared/protocol.ts"),
    read("../src/agent/runtime/session-title-summarize.ts"),
  ]);
test("session title summarization is wired through the full desktop path", () => {
  assert.match(protocol, /sessionSummarizeTitle: "dcode\/session\/summarizeTitle"/);
