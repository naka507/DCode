...
} from "../../../src/agent/runtime/extensions/discovery.js";
...
describe("discovery", () => {
  ...
  it("orders user, project, manual and dedupes by realpath", () => {
    ...
    // .dcode/extensions is also discovered and wins over .pi/extensions with same id
    const dcodeProject = join(root, "proj-dcode");
    file(join(dcodeProject, ".dcode", "extensions", "b.ts"));
    const dcodeSpecs = discoverTrustedExtensions({
      agentDir,
      projectPath: dcodeProject,
    });
    expect(dcodeSpecs.map((s) => [s.label, s.source])).toEqual([
      ["a", "user"],
      ["b", "project"],
    ]);
  });
