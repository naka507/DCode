...
} from "../../src/agent/runtime/mode-prompts.js";
...
describe("mode-specific system prompts", () => {
  it("composes Plan instructions over the shared runtime prompt", () => {
    ...
    expect(prompt).toContain("host writes a new .dcode/plan artifact");
  ...
  it("composes Goal instructions asking for a contract, not steps", () => {
    ...
    expect(prompt).toContain("host writes a new .dcode/goal artifact");
