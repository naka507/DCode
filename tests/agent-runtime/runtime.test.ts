...
import { formatSessionMessage, type SessionMessageOrigin } from "@dcode/shared";
...
import { COMPACTION_SUMMARY_MAX_RETRIES } from "../../src/agent/runtime/compaction-summary-input.js";
import type { ProjectInstructions } from "../../src/agent/runtime/project-instructions.js";
import { classifyAgentError } from "../../src/agent/runtime/agent-errors.js";
import {
  PROVIDER_RATE_LIMIT_MAX_RETRIES,
  PROVIDER_TRANSIENT_MAX_RETRIES,
} from "../../src/agent/runtime/provider-retry.js";
import { getGlobalDispatcher } from "undici";
import { describeProviderFetchFailure } from "../../src/agent/runtime/provider-transport-recovery.js";
/**
...
vi.mock("../../src/agent/runtime/subagent.js", async (importOriginal) => {
  const actual = await importOriginal<typeof import("../../src/agent/runtime/subagent.js")>();
  return {
    ...actual,
    SubagentRun: class {
...
function createRuntime(
  overrides: Partial<{
    ...
    projectPath: string;
    scratchDir: string;
    projectInstructions: import("../../src/agent/runtime/project-instructions.js").ProjectInstructions;
    projectMemory: string;
    pluginTools: PluginToolDef[];
    subagents: SubagentDefinition[];
    subagentProviders: Record<string, RuntimeProviderConfig>;
    subagentModelKeys: string[];
    pluginSkills: import("../../src/agent/runtime/plugin-skills-prompt.js").PluginSkillDef[];
...
describe("DesktopAgentRuntime plan transitions", () => {
  it("guards transition batches and terminates after durable plan submission", async () => {
    ...
    const proposal = {
      ...
      artifact: {
        relativePath: ".dcode/plan/proposal-1.md",
        sha256: "abc123",
        sizeBytes: 31,
      },
      version: 1,
  ...
  it("routes the Goal contract through the same host approval with kind goal", async () => {
    ...
    const proposal = {
      ...
      artifact: {
        relativePath: ".dcode/goal/proposal-goal-1.md",
        sha256: "def456",
        sizeBytes: 42,
      },
      version: 1,
  ...
  it("re-runs a silent turn during approved plan execution", async () => {
    ...
    const execution: PlanExecution = {
      ...
      question: "Proceed?",
      artifact: {
        relativePath: ".dcode/plan/proposal-silent.md",
        sha256: "abc123",
        sizeBytes: 24,
      },
      targetPermissionMode: "auto",
  ...
  it("re-runs a progress-only turn once during approved plan execution", async () => {
    ...
    const execution: PlanExecution = {
      ...
      question: "Proceed?",
      artifact: {
        relativePath: ".dcode/plan/proposal-progress.md",
        sha256: "abc123",
        sizeBytes: 24,
      },
      targetPermissionMode: "auto",
  ...
  it("chains silent recovery after a progress-only turn", async () => {
    ...
    const execution: PlanExecution = {
      ...
      question: "Proceed?",
      artifact: {
        relativePath: ".dcode/plan/proposal-progress-silent.md",
        sha256: "abc123",
        sizeBytes: 24,
      },
      targetPermissionMode: "auto",
  ...
  it("does not re-run a normal approved plan final report", async () => {
    ...
    const execution: PlanExecution = {
      ...
      question: "Proceed?",
      artifact: {
        relativePath: ".dcode/plan/proposal-final.md",
        sha256: "abc123",
        sizeBytes: 24,
      },
      targetPermissionMode: "auto",
  ...
  it("starts each approved plan execution from clean recovery state", async () => {
    ...
    const execution: PlanExecution = {
      ...
      question: "Proceed?",
      artifact: {
        relativePath: ".dcode/plan/proposal-clean.md",
        sha256: "abc123",
        sizeBytes: 24,
      },
      targetPermissionMode: "auto",
      ...
      runtime.executeApprovedPlan(
        {
          ...
          question: "q",
          plan: "p",
          artifact: { relativePath: ".dcode/plan.md" },
...
describe("DesktopAgentRuntime subagents", () => {
  ...
  it("feeds finished reports incrementally when multiple subagents run concurrently", async () => {
    const runtime = createRuntime({ subagents: [explorer] });
    subagentRuns.calls.length = 0;
    subagentRuns.instances.length = 0;
    subagentRuns.deferred = true;
    ...
    const started1 = await tool.execute("task-1", {
      agent: "explorer",
      task: "Task 1",
    });
    const id1 = (started1.details as any).delegationId as string;

    const started2 = await tool.execute("task-2", {
      agent: "explorer",
      task: "Task 2",
    });
    const id2 = (started2.details as any).delegationId as string;

    const resumePromise = (runtime as any).resumeAfterDelegations();

    // Settle only the first subagent
    subagentRuns.resolveRun!({
      agentName: "explorer",
      status: "completed",
      report: "First subagent result",
      turns: 1,
      toolCalls: 1,
    });

    // Verify first prompt was called with the first report while second is still running
    await vi.waitFor(() => {
      expect(prompt).toHaveBeenCalledTimes(1);
    });
    const firstPromptText = String(
      (prompt.mock.calls as unknown as unknown[][])[0]?.[0] ?? "",
    );
    expect(firstPromptText).toContain("First subagent result");
    expect(firstPromptText).toContain("Still running:");
    expect(firstPromptText).toContain(id2);
    expect((runtime as any).delegations.get(id1).reportDelivered).toBe(true);
    expect((runtime as any).delegations.get(id2).reportDelivered).toBe(false);
    expect((runtime as any).keepTurnOpenForDelegates()).toBe(true);

    // Now settle the second subagent
    subagentRuns.resolveRun!({
      agentName: "explorer",
      status: "completed",
      report: "Second subagent result",
      turns: 1,
      toolCalls: 1,
    });
    ...
    const secondPromptText = String(
      (prompt.mock.calls as unknown as unknown[][])[1]?.[0] ?? "",
    );
    expect(secondPromptText).toContain("Second subagent result");
    expect(secondPromptText).not.toContain("Still running:");
    expect((runtime as any).delegations.get(id2).reportDelivered).toBe(true);
  ...
  it("defaults TaskWait to mode 'any' to converge on the first completed subagent", async () => {
    ...
    const started1 = await task.execute("task-1", {
      agent: "explorer",
      task: "Task 1",
    });
    const id1 = (started1.details as any).delegationId as string;

    const started2 = await task.execute("task-2", {
      agent: "explorer",
      task: "Task 2",
    });
    const id2 = (started2.details as any).delegationId as string;

    const waitPromise = wait.execute("wait-1", { delegationIds: [id1, id2] });

    // Settle only subagent 1
    subagentRuns.resolveRun!({
      agentName: "explorer",
      status: "completed",
      report: "Subagent 1 finished early",
      turns: 1,
      toolCalls: 1,
    });

    const result = await waitPromise;
    expect(result.content[0].text).toContain("Subagent 1 finished early");
    expect(result.content[0].text).toContain("Converged after 1 of 2 finished");
    expect((runtime as any).delegations.get(id1).status).toBe("completed");
    expect((runtime as any).delegations.get(id2).status).toBe("running");

    // Clean up subagent 2
    subagentRuns.resolveRun!({
      agentName: "explorer",
      status: "completed",
      report: "Subagent 2 finished later",
      turns: 1,
      toolCalls: 1,
    });
    await vi.waitFor(() => {
      expect((runtime as any).delegations.get(id2).status).toBe("completed");
    });
