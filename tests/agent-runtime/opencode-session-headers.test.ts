...
import { completeOneShot } from "../../src/agent/runtime/one-shot-complete.js";
import {
...
} from "../../src/agent/runtime/opencode-session-headers.js";
...
describe("mergeOpenCodeSessionHeaders", () => {
  it("injects session, client, and user-agent headers", () => {
    expect(
      ...
      "User-Agent": `dcode/${APP_VERSION}`,
    });
    expect(OPENCODE_USER_AGENT).toBe(`dcode/${APP_VERSION}`);
  });
...
describe("OpenCode header call-site wiring", () => {
  it("is applied on session, subagent, and one-shot streams", () => {
    const sources = [
      readFileSync(new URL("../../src/agent/runtime/runtime.ts", import.meta.url), "utf8"),
      readFileSync(new URL("../../src/agent/runtime/subagent-model-binding.ts", import.meta.url), "utf8"),
      readFileSync(new URL("../../src/agent/runtime/one-shot-complete.ts", import.meta.url), "utf8"),
    ];
