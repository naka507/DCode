...
} from "../../src/agent/runtime/prompt-templates.js";
...
describe("loadComposerTemplates", () => {
  ...
  beforeAll(async () => {
    root = await mkdtemp(join(tmpdir(), "dcode-templates-"));
  ...
  it("derives the project dir from the workspace root", () => {
    ...
    expect(dirs.project).toBe(join("/tmp/ws", ".dcode", "prompts"));
    expect(dirs.user.endsWith(join(".pi", "agent", "prompts"))).toBe(true);
    expect(composerTemplateDirs(null).project).toBeUndefined();
  });

  it("falls back to legacy .pi/prompts if .dcode/prompts does not exist", async () => {
    const ws = await mkdtemp(join(tmpdir(), "dcode-legacy-ws-"));
    try {
      const piPrompts = join(ws, ".pi", "prompts");
      await mkdir(piPrompts, { recursive: true });
      expect(composerTemplateDirs(ws).project).toBe(piPrompts);

      // but if .dcode/prompts exists, it wins
      const dcodePrompts = join(ws, ".dcode", "prompts");
      await mkdir(dcodePrompts, { recursive: true });
      expect(composerTemplateDirs(ws).project).toBe(dcodePrompts);
    } finally {
