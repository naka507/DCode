...
import { activeNodeTransportRoute } from "../../src/agent/runtime/node-proxy.js";
import { describeProviderFetchFailure } from "../../src/agent/runtime/provider-transport-recovery.js";
import type { ClassifiedAgentError } from "../../src/agent/runtime/agent-errors.js";
...
describe("stream idle watchdog", () => {
  it("reads the idle budget from the environment with a safe default", () => {
    ...
    try {
      delete process.env.DCODE_STREAM_IDLE_TIMEOUT_MS;
      expect(streamIdleTimeoutMs()).toBe(STREAM_IDLE_TIMEOUT_DEFAULT_MS);
      process.env.DCODE_STREAM_IDLE_TIMEOUT_MS = "45000";
      expect(streamIdleTimeoutMs()).toBe(45_000);
      process.env.DCODE_STREAM_IDLE_TIMEOUT_MS = "0";
      expect(streamIdleTimeoutMs()).toBe(0);
      process.env.DCODE_STREAM_IDLE_TIMEOUT_MS = "not-a-number";
      expect(streamIdleTimeoutMs()).toBe(STREAM_IDLE_TIMEOUT_DEFAULT_MS);
    } finally {
      if (previous === undefined) {
        delete process.env.DCODE_STREAM_IDLE_TIMEOUT_MS;
      } else {
        process.env.DCODE_STREAM_IDLE_TIMEOUT_MS = previous;
      }
    }
  });
  ...
  it("clamps a positive override up to the retry backoff floor", () => {
    ...
    try {
      ...
      process.env.DCODE_STREAM_IDLE_TIMEOUT_MS = "1000";
