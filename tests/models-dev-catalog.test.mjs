...
import { apiStyleForAdapter, modelIdsMatch } from "@dcode/shared";
...
} from "../src/main/models-dev-catalog.ts";
...
test("one read with more distinct keys than any cache budget does not rescan a resolved key", async (t) => {
  ...
  // Interleave the exact key that was resolved first among a burst of distinct
  // keys far larger than the former 1,024-entry per-key eviction cache.
...
// Covers the Settings default-picker change: a `toggle` reasoning option must
// keep mapping `off` to `none`, so host-core's widened session CHECK can never
// be reached by a toggle ladder.
...
/*
  The bundled release snapshot is the only data source: the runtime remote
  refresh was removed, so nothing in this module may reach a models.dev URL.
  Both levels are asserted — no method to call, and no fetch in the module.
*/
test("the catalog has no network refresh path", async () => {
  const dir = await mkdtemp(join(tmpdir(), "pi-models-dev-no-network-"));
  const catalogPath = join(dir, "api.json");
  await writeFile(catalogPath, JSON.stringify(catalogFixture), "utf8");
  try {
    const catalog = new ModelsDevCatalog({ catalogPath });
    assert.equal(catalog.refresh, undefined, "the remote refresh method is gone");
    assert.equal(await catalog.ensureLoaded(), true);
    assert.equal(catalog.getStatus().source, "bundled");
    assert.equal(catalog.getStatus().fetchedAt, undefined, "nothing is ever fetched");
    assert.equal(
      catalog.findModel({ vendorKey: "anthropic", modelId: "claude-opus-4.6" })?.family,
      "claude-opus",
    );
    const moduleSource = await readFile(
      new URL("../src/main/models-dev-catalog.ts", import.meta.url),
      "utf8",
    );
    assert.doesNotMatch(moduleSource, /models\.dev\/api\.json/);
...
test("an unreadable bundled snapshot leaves the catalog empty and reports the error", async () => {
  const dir = await mkdtemp(join(tmpdir(), "pi-models-dev-failure-"));
  const catalogPath = join(dir, "missing.json");
  try {
    const catalog = new ModelsDevCatalog({ catalogPath });
    ...
    assert.ok(status.lastError, "the snapshot read failure is reported");
    assert.equal(
      catalog.findModel({ vendorKey: "anthropic", modelId: "claude-opus-4.6" }),
      undefined,
    );
