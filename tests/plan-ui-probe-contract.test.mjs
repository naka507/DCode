...
const probeSource = await readFile(
  new URL("../src/main/plan-ui-probe.ts", import.meta.url),
  "utf8",
);
...
test("Plan UI probe is Main-only, explicitly gated, and reuses the live Host", () => {
  ...
  assert.match(probeSource, /DCODE_PLAN_UI_PROBE/);
  assert.match(probeSource, /if \(process\.env\.DCODE_PLAN_UI_PROBE !== "1"\) return/);
  assert.match(probeSource, /__DCODE_PLAN_UI_PROBE/);
  for (const method of [
    "workspace\\.set",
    "session\\.create",
    "session\\.beginTurn",
...
test("runtime identity is a probe-gated WeakMap identity for the live runtime object", () => {
  assert.match(sidecarSource, /DCODE_PLAN_UI_PROBE/);
...
test("live setup is Main-env gated and the harness never sends credentials through CDP", () => {
  assert.match(probeSource, /DCODE_TEST_API_KEY/);
  assert.match(probeSource, /DCODE_TEST_BASE_URL/);
  assert.match(probeSource, /DCODE_TEST_MODEL/);
  ...
  assert.doesNotMatch(
    harnessSource,
    /JSON\.stringify\([\s\S]{0,240}DCODE_TEST_API_KEY/,
  );
});
...
test("Plan UI harness uses the second inspector and never starts a fixture Host", () => {
  ...
  assert.match(harnessSource, /globalThis\.__DCODE_PLAN_UI_PROBE/);
