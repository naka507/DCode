...
const hostProcessEntry = join(desktopRoot, "src/main/plugin-host-process.ts");
...
process.env.DCODE_DATA_DIR = mkdtempSync(join(tmpdir(), "pi-fs-scope-data-"));
...
test("a symlink inside the workspace cannot carry a read out of it", async (t) => {
  ...
  // Creating a link needs a privilege this host may withhold (Windows without
  // Developer Mode or elevation), so an unusable host skips instead of failing
  // — the same guard fs-panel-guard.test.mjs uses for the same reason.
  try {
