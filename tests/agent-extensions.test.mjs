...
} = await import("../src/main/agent-extensions.ts");
const {
  IMPORTED_PLUGIN_WRAPPER_SOURCE,
  repairImportedExtensionWrapper,
} = await import("../src/main/imported-plugin-wrapper.ts");
const { withRegistryOnlyProxy } = await import("../src/main/npm-registry-proxy.ts");
...
test("repairImportedExtensionWrapper migrates the generated main.js no-op in place", () => {
  ...
  writeFileSync(join(dir, "main.js"), "// Generated by dcode: this plugin only contributes agent extensions.\nmodule.exports = {};\n");
...
test("default runner caps captured stderr and escalates the timeout kill", async () => {
  const { defaultDependencyRunner } = await import("../src/main/agent-extensions.ts");
...
test("one install deadline covers both npm stages instead of restarting per stage", async () => {
  const root = mkdtempSync(join(tmpdir(), "pi-ax-deadline-"));
  const dir = join(root, "bounded");
  mkdirSync(dir, { recursive: true });
  writeFileSync(
    join(dir, "package.json"),
    JSON.stringify({ name: "x", dependencies: { "some-dep": "^1" } }),
  );

  // An already-expired budget must not start npm at all.
  const neverCalled = [];
  const expired = await installExtensionDependencies(dir, {
    timeoutMs: 0,
    runner: async (...args) => {
      neverCalled.push(args);
      return { code: 0, stderr: "" };
    },
  });
  assert.equal(expired.state, "failed");
  assert.match(expired.error, /exceeded its time budget/);
  assert.deepEqual(neverCalled, [], "an exhausted budget never spawns npm");

  // The first stage consuming the whole budget leaves nothing for `npm ci`,
  // which is what "one deadline" means: the clock is not restarted per stage.
  const calls = [];
  const spent = await installExtensionDependencies(dir, {
    timeoutMs: 40,
    runner: async () => {
      calls.push(Date.now());
  ...
  assert.equal(calls.length, 1, "the second stage never runs once the budget is spent");
  assert.equal(spent.state, "failed");
  assert.match(spent.error, /exceeded its time budget/);
});
