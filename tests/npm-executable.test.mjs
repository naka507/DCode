/**
 * npm executable selection contract.
 *
 * The suite pins the selected-npm path: an explicitly chosen executable is
 * validated through its own directory PATH, never inherits the desktop
 * process's secrets or npm config, and a launch failure is reported as
 * `npm-unavailable` while a registry failure is not.
 *
 * Layout and platform notes:
 *   - `defaultDependencyRunner` and the npm modules live under `src/main/`.
 *   - This file registers the TS hooks itself, so it stays runnable on its own
 *     via `node --test tests/npm-executable.test.mjs`.
 *   - The POSIX-only cases (symlinked npm, Node version without a leading `v`,
 *     orphaned descendant kill) carry a documented skip on Windows, matching
 *     this tree's existing POSIX-skip pattern.
 */
...
const { defaultDependencyRunner, validateNpmExecutable } = await import(
  "../src/main/npm-executable.ts"
);

register(new URL("./helpers/ts-import-hooks.mjs", import.meta.url));
const { installExtensionDependencies } = await import("../src/main/npm-installer.ts");
const windows = process.platform === "win32";

/** `shell: false` cannot execute a shebang fixture on Windows (ENOENT). */
const NEEDS_POSIX = {
  skip: "the fixture is a `#!/usr/bin/env node` script spawned with `shell: false`, which Windows cannot execute (ENOENT)",
};
...
test("symlinked npm keeps the selected bin directory, not the real npm-cli directory", NEEDS_POSIX, async (t) => {
  const f = fixture(t);
...
test("Node validation requires the leading v", NEEDS_POSIX, async (t) => {
  const f = fixture(t);
  rmSync(f.node);
...
test("short install budgets cap validation and include its time in both install stages", async (t) => {
  ...
  assert.equal(result.reason, "npm-unavailable");
  // The property is that the 150ms budget bounds the run instead of the 10s
  // validation default. Windows adds a fixed cost here: killing the timed-out
  // probe's process tree through `taskkill /T /F` measured 1.5-2.5s on an idle
  // machine, so the bound stays platform-aware while remaining far below 10s.
  const ceiling = windows ? 6_000 : 2_000;
  assert.ok(Date.now() - started < ceiling, `validation is bounded by the caller budget, not the 10s default (${Date.now() - started}ms)`);
...
test("timeout kills resistant descendants after their group leader exits with ignored stdio", { ...NEEDS_POSIX, timeout: 10_000 }, async (t) => {
  const f = fixture(t);
