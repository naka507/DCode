/**
 * Tool-call presentation blocks.
 *
 * Tool-call presentation blocks: a pure-function test against
 * `src/renderer/lib/tool-presentation.ts`. Two things differ from the module
 * path's original spelling:
 *
 *  - the module path (`src/lib/…` → `src/renderer/lib/…`);
 *  - the dynamic-import form: `test/helpers/ts-import-hooks.mjs` lives at
 *    `tests/helpers/ts-import-hooks.mjs` here.
 *
 * Every `test(...)` keeps its name and expectations. Nothing was dropped: the
 * module still exports `buildDiffLines`, `buildToolPresentation`,
 * `hasToolDetails`, `langForPath`, `runOutcome`, `toolResultChips` and
 * `toolResultPayload`.
 */
...
const {
  ...
  toolResultChips,
  toolResultPayload,
} = await import("../src/renderer/lib/tool-presentation.ts");
...
test("Write shows the written content and a size chip", () => {
  ...
  assert.deepEqual(toolResultChips(message), [{ role: "size", text: "2.0 KB" }]);
});
