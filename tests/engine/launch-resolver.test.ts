...
import { createHeadlessLaunchResolver, type HostProviderRecord } from "../../src/engine/launch-resolver.js";
...
describe("createHeadlessLaunchResolver", () => {
  it("resolves the session's provider, its secret, the shell, skills and project memory", async () => {
    ...
    // `scratchDir` is built with `node:path`'s platform `join`, so the expected
    // value is derived the same way. The three segments stay independently
    // specified, so this still pins the dataDir root, the `scratch` segment and
    // the session id.
