...
const { scanModelConfigs } = await import("../src/main/importers/model-config.ts");
...
test("settings import and protocol expose model-config import independently of sessions", async () => {
  ...
  const apiSource = await readFile(new URL("../src/renderer/lib/api.ts", import.meta.url), "utf8");
  ...
  assert.match(protocol, /dcode\/modelConfig\/importScan/);
...
test("scanModelConfigs carries the per-model hosted web search opt-in through both spellings", async () => {
  // `003bc0ae` added the mapping to `bindingFromGenericModel`. Both spellings
  // are accepted because the same JSON is written by this app
  // (`nativeWebSearch`) and read back out of host-core rows / hand-edited Pi
  // configs (`native_web_search`, the Rust field name). A dropped opt-in is
  // silent: the provider simply never attaches the search tool.
  const home = await mkdtemp(join(tmpdir(), "pi-native-web-search-"));
  await mkdir(join(home, ".pi", "agent"), { recursive: true });
  await writeFile(
    join(home, ".pi", "agent", "models.json"),
    JSON.stringify({
      providers: {
        searchable: {
          ...
          apiKey: "sk-search",
          models: [
            { id: "opted-in", nativeWebSearch: true },
            { id: "rust-spelling", native_web_search: true },
            { id: "opted-out", nativeWebSearch: false },
            { id: "unset" },
          ],
        },
      },
  ...
  const drafts = await scanModelConfigs({ homeDir: home, env: {} });
  const provider = drafts.find((draft) => draft.externalId === "searchable");
  assert.ok(provider, "the provider row must be scanned");
  const byId = new Map(provider.models.map((binding) => [binding.id, binding]));
  assert.equal(byId.get("opted-in")?.nativeWebSearch, true);
  assert.equal(byId.get("rust-spelling")?.nativeWebSearch, true);
  // Absent stays absent rather than becoming an explicit `false`: the field is
  // a two-state opt-in and `false` would serialize as a stored decision.
  assert.equal(byId.get("opted-out")?.nativeWebSearch, undefined);
  assert.equal(byId.get("unset")?.nativeWebSearch, undefined);
  assert.ok(
    !Object.hasOwn(byId.get("opted-out"), "nativeWebSearch"),
    "a declined opt-in must not be written as a key",
  );
});
