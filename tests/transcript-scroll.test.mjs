/**
 * Transcript scroll reducer contract.
 *
 * The module lives at `src/renderer/lib/transcript-scroll.ts` and carries the
 * additions of two commits:
 *
 *  - `d68ec683` ("keep session opens at the latest turn") added
 *    `transcriptHasLayout` and `isHistoryRevealPosition` coverage.
 *  - `1bd427ea` ("advance history for a gesture at the near-top band") added
 *    the one-pixel-overflow slack and the anywhere-in-the-band suppression.
 *
 * The only translation is the module path (`src/lib` → `src/renderer/lib`) and
 * the import form: the `.ts` source is imported directly, which is what
 * `node --experimental-transform-types` in this tree's `npm test` script does
 * too. Every `test(...)` below keeps its name and expectations.
 */
...
import {
  ...
  transcriptHasLayout,
} from "../src/renderer/lib/transcript-scroll.ts";
