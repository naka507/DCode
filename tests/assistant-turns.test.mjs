...
const {
  ...
  subagentRunsEqual,
} = await import("../src/renderer/lib/assistant-turns.ts");
...
test("surfaces each hosted search round as its own activity row", () => {
  // The pre-revert test asserted one row per *message* carrying a flat
  // `{queries, sources}` aggregate; `86398e48` deleted it and `2e763893` did not
  // re-add a replacement here. One row per provider round is the new contract,
  // and nothing else in the suite executes it (the vitest suites cover the
  // runtime half, not the renderer).
  ...
  assert.deepEqual(
    activity.items.map((item) => item.round.id),
    ["ws_1", "ws_2"],
  );
  assert.equal(activity.items[0].message, assistant);
  assert.equal(activity.items[0].round.query, "dcode web search");
  assert.equal(activity.items[0].round.sources[0].url, "https://example.com");
  // Each row is keyed by message *and* round, or two rounds of one message
  // would collapse into a single keyed row.
  assert.deepEqual(
    renderActivityRows(activity.items).map((row) => row.key),
    ["hosted-search-a1-ws_1", "hosted-search-a1-ws_2"],
  );
});

test("a hosted search message with no text still renders its search rows", () => {
  // `isVisibleMessage` gained `!message.hostedSearch` in the same change: a
  // search-only assistant bubble is not an empty message, so it must not be
  // dropped along with the rounds it carries.
  const assistant = message("a1", "assistant", "", {
    hostedSearch: {
      status: "searching",
      rounds: [{ id: "ws_1", status: "searching", query: "q", sources: [] }],
    },
  });
  const { entries } = buildTranscriptEntries([
    message("u1", "user", "Search that"),
  ...
  const turn = entries.find((entry) => entry.kind === "assistant-turn");
  assert.ok(turn, "a search-only assistant bubble must still open a turn");
  const activity = turn.parts.find((part) => part.kind === "activity");
  assert.equal(activity?.items[0]?.kind, "hostedSearch");
  assert.equal(activity.items[0].round.id, "ws_1");
});
