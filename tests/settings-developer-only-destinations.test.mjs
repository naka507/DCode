...
import {
  ...
  searchSettings,
  visibleSettingsNav,
} from "../src/renderer/lib/settings-search.ts";
...
const searchDialog = readFileSync(
  new URL("../src/renderer/components/SearchDialog.vue", import.meta.url),
  "utf8",
);
...
test("the rail, the page, and the search dialog follow developer mode", () => {
  assert.match(settingsPage, /const developerMode = computed\(\(\) => settings\.value\?\.developerMode === true\)/);
  assert.match(settingsPage, /visibleSettingsNav\(developerMode\.value\)/);
  assert.match(settingsPage, /isSettingsDestinationHidden\(tab\.value, developerMode\.value\)/);
  assert.match(settingsPage, /setSettingsTab\("general"\)/);
  assert.match(settingsPage, /<RemoteHostsPage v-if="tab === 'remoteHosts' && !tabHidden" \/>/);
  assert.match(searchDialog, /searchSettings\(query\.value, t, \{ developerMode: developerMode\.value \}\)/);
});
