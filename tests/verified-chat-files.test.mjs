...
const { splitChatText } = await import("../src/renderer/lib/chat-links.ts");
const {
  chatFileCandidates,
  createChatFileVerificationQueue,
  verifiedChatSegments,
  verifyChatFiles,
} = await import("../src/renderer/lib/verified-chat-files.ts");

/**
 * Covers the regression set for issue #649 (sha a1db7e99). Only the import
 * paths differ: the module lives under `src/renderer/lib/`. The module itself
 * is framework-free, so no assertion needed retranslating.
 */
