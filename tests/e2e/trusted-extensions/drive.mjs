...
const invoke = (channel, ...args) => tool("dcode_invoke", { operation: channel, args }).then((r) => (r && typeof r === "object" && "result" in r ? r.result : r));
...
for (let i = 0; i < 60 && answeredNow() < 3; i++) {
  ...
  for (const line of lines) {
    ...
    await invoke("extensions/ui/respond", { promptId: m[1], value: answers[m[2]], confirm: true }).catch(async () => {
      await tool("dcode_invoke", { operation: "extensions/ui/respond", args: [{ promptId: m[1], value: answers[m[2]] }], confirm: true });
    });
  }
}
let runResult;
...
// registerProvider is the compatibility alias: the same plugin-owned
