...
optional `DCODE_HOST_BIN` environment variable selects a host binary.
...
`tests/agent-runtime/extensions/bundle.test.ts`.

For manual inspection, the lower-level steps remain available:

```bash
E2E_ROOT=/tmp/pi-ext-e2e STUB_PORT=47123 \
  HOST_BIN="$PWD/target/debug/DCore" \
  node tests/e2e/trusted-extensions/seed.mjs
```
