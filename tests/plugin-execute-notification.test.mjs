...
const { createHostRuntime } = await import("../src/main/runtime/host.ts");
const { createSessionCoordination } = await import(
  "../src/main/runtime/session-coordination.ts"
);
...
// host-core's payload literal lives in the sibling dcore checkout, so a machine
// without that checkout skips the cross-language contract instead of failing.
const skip =
  rpcPath === null ? "needs the dcore checkout beside dcode" : false;
const rustSource = skip ? "" : readFileSync(rpcPath, "utf8");
...
// A skipped run has no Rust source to parse, so the field list stays empty
// instead of tripping the "notification is gone" assertion at load time.
const FIELDS = skip ? [] : pluginExecutePayloadFields(rustSource);
...
test("the host-core notification carries every field the receiver reads", { skip }, () => {
...
test("the live turn's payload runs the plugin tool and is answered", { skip }, async () => {
...
test("a payload naming a turn that no longer owns the session is refused", { skip }, async () => {
...
test("a payload for a turn that already ended is refused", { skip }, async () => {
...
test("a payload without a turn identity is refused rather than executed", { skip }, async () => {
...
test("a payload without a session identity is refused on the same grounds", { skip }, async () => {
  const f = fixture({ activeTurn: TURN_ID });
...
test("an mcprefixed tool keeps its own path", { skip }, async () => {
  const f = fixture({ activeTurn: TURN_ID });
