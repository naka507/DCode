...
import {
  ...
  releaseSessionPane,
  retainSessionPane,
} from "../src/renderer/lib/session-panes.ts";
...
test("the store keeps the retained panes in step with the live projection", () => {
  ...
  assert.match(store, /store\.subscribe\(\(state, previous\) => \{/);
  // The subscription must be registered in the same boot step that commits the
  // initial state, so no commit can slip through before panes are mirrored.
  const commitAt = store.indexOf("store.commitInitialState(buildInitialState())");
  const subscribeAt = store.indexOf("store.subscribe((state, previous) => {");
  assert.ok(commitAt >= 0 && subscribeAt > commitAt, "commit before subscribe");
