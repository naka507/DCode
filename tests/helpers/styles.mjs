...
const STYLE_ROOT = new URL("../../src/renderer/styles/", import.meta.url);
const ENTRY = new URL("globals.css", STYLE_ROOT);
  ...
  const parts = new Map(
    await Promise.all(
      names.map(async (name) => [
        name,
        await readFile(new URL(name, STYLE_ROOT), "utf8"),
      ]),
    ),
  );
...
export function loadStylesSync() {
  ...
  return entry.replace(IMPORT_RE, (_line, name) =>
    readFileSync(new URL(name, STYLE_ROOT), "utf8"),
  );
}
