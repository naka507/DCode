...
test("logger routes records by category and keeps child stderr line-safe", async () => {
  const dataDir = await mkdtemp(join(tmpdir(), "dcode-logger-"));
...
test("logger suppresses info console mirroring in production and mirrors full records in development", async () => {
  const dataDir = await mkdtemp(join(tmpdir(), "dcode-logger-console-"));
  const previousNodeEnv = process.env.NODE_ENV;
...
test("logger bounds and redacts messages, paths, credentials, and arbitrary data", async () => {
  const dataDir = await mkdtemp(join(tmpdir(), "dcode-logger-redaction-"));
  try {
    const logger = new Logger(dataDir, "debug");
    ...
    // The redaction contract is that the data directory collapses to
    // `<data-dir>`; the separators after it are the host path's own, so the
    // comparison folds them instead of pinning one platform's spelling.
    assert.equal(
      record.data.path.replace(/\\/g, "/"),
      "<data-dir>/workspace/private.txt",
    );
...
test("logger console mirror never throws when stdout is a broken pipe", async () => {
  const dataDir = await mkdtemp(join(tmpdir(), "dcode-logger-epipe-"));
  const previousLog = console.log;
  const previousError = console.error;
  const previousNodeEnv = process.env.NODE_ENV;
