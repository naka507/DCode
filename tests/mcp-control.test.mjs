...
test("the plugin and MCP paths share desktop operation validation", async () => {
  ...
  assert.deepEqual(calls, [
    { channel: "dcode/project/set", args: ["/tmp/project"] },
    { channel: "dcode/session/delete", args: ["s1"] },
  ]);
});
...
test("local MCP control server authenticates, discovers, and invokes desktop operations", async (t) => {
  ...
  const server = new McpControlServer({
    ...
    invoke: async (channel, args) => {
      calls.push({ channel, args });
      if (channel === "dcode/app/getVersion") {
        return { name: "dcode", version: "test" };
      }
      if (channel === "dcode/project/set") {
        return { workspace: { path: args[0], name: "fixture" } };
      }
      if (channel === "dcode/session/create") {
  ...
  assert.equal(initialized.body.result.serverInfo.name, "dcode");
  ...
  assert.ok(toolNames.includes("dcode_invoke"));
  ...
  assert.equal(calls.some((call) => call.channel === "dcode/session/delete"), false);
  ...
  assert.equal(calls.at(-1).channel, "dcode/session/delete");
  ...
  assert.equal(calls.some((call) => call.channel === "dcode/plans/resolve"), false);
  ...
  assert.equal(calls.some((call) => call.channel === "dcode/session/configure"), false);
  ...
  assert.deepEqual(calls.at(-1), {
    channel: "dcode/session/configure",
...
test("the MCP control connection file is marked inactive on shutdown", async () => {
  ...
  const server = new McpControlServer({
    dataDir,
    port: 0,
    channels: { appGetVersion: "dcode/app/getVersion" },
    invoke: async () => ({}),
  });
  const info = await server.start();
  await server.stop();
...
test("plugin-only session collaboration operations stay off the external MCP surface", async (t) => {
  ...
  const invokeTool = listed.body.result.tools.find((tool) => tool.name === "dcode_invoke");
  ...
  const hidden = await post(
    ...
    info.token,
    {
      ...
      id: 4,
      method: "tools/call",
      params: { name: "dcode_invoke", arguments: { operation: "session/collaboration/list", args: [] } },
    },
    { "Mcp-Session-Id": sessionId },
  );
  ...
  const known = await post(
    ...
    info.token,
    {
      ...
      id: 5,
      method: "tools/call",
      params: { name: "dcode_invoke", arguments: { operation: "app/getVersion", args: [] } },
    },
    { "Mcp-Session-Id": sessionId },
  );
  ...
  assert.deepEqual(known.body.result.structuredContent, {
    ...
    result: { ok: true, channel: "dcode/app/getVersion" },
  });
  assert.deepEqual(calls.at(-1), { channel: "dcode/app/getVersion", args: [] });
});
