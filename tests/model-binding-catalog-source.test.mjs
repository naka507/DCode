...
async function fixtureRuntime(t) {
  ...
  const dir = await mkdtemp(join(tmpdir(), "pi-model-binding-catalog-source-"));
...
test("a catalog-sourced binding adopts a models.dev correction", async (t) => {
...
test("a hand-edited window survives a models.dev correction", async (t) => {
  const runtime = await fixtureRuntime(t);
  const edited = {
    id: "terra",
...
test("a hand-edited window that equals the generic seed is still the user's", async (t) => {
  const runtime = await fixtureRuntime(t);
...
test("unmarked records keep the rule they were written under", async (t) => {
  const runtime = await fixtureRuntime(t);
  const legacySeed = {
    id: "terra",
    contextWindow: 128_000,
...
test("a catalog value leaves the binding marked as catalog-sourced", async (t) => {
  const runtime = await fixtureRuntime(t);
  const legacySeed = {
    id: "terra",
    contextWindow: 128_000,
...
test("a model the catalog does not publish keeps its stored window", async (t) => {
  const runtime = await fixtureRuntime(t);
  const binding = {
    id: "unpublished",
