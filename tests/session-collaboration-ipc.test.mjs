...
import { ErrorCodes } from "../src/shared/errors.ts";
...
const collaboration = load("../src/main/services/session-collaboration.ts", {
  "node:crypto": crypto,
  "../agent-host-bridge": {},
  "../plugin-agent-complete": {},
});
const { registerSessionIpc } = load("../src/main/ipc/session-ipc.ts", {
  electron: { shell: {} },
  "node:fs": fs,
  "node:path": path,
  "@dcode/shared": shared,
...
function rendererApi(invoke) {
  ...
  load("../src/preload/index.ts", {
    ...
    "@dcode/shared": shared,
    "@dcode/shared/protocol": shared,
  });
  // The api module now de-proxies its arguments before they cross the bridge,
  // so its real dependency is loaded rather than stubbed: the stub below
  // rejects any `require` it was not told about, which is the point of it.
  const ipcPayload = load("../src/renderer/lib/ipc-payload.ts", { vue });
  const { api } = load(
    "../src/renderer/lib/api.ts",
    { "@dcode/shared": shared, "./ipc-payload": ipcPayload },
    { window: { dcode: bridge } },
  );
  return { api, bridge };
}
