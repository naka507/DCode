...
const hostSource = await readFile(
  new URL("../src/main/plugin-panel-host.ts", import.meta.url),
  "utf8",
);
const preloadSource = await readFile(
  new URL("../src/preload/plugin-panel.ts", import.meta.url),
  "utf8",
);
const chromeSource = await readFile(
  new URL("../src/shared/plugin-panel-chrome.ts", import.meta.url),
  "utf8",
);

// The bundled catalog lives in the sibling dcore crate, so a missing sibling
// skips the bundled-panel contract instead of failing on a missing neighbour.
const hostCorePluginPath = dcorePath(
  "crates/host-core/src/plugins/marketplace/catalog.rs",
);

const skip =
  hostCorePluginPath === null
    ? "needs the dcore checkout beside dcode"
    : false;

const hostCorePluginSource = skip ? "" : await readFile(hostCorePluginPath, "utf8");
const bundledPanelSources = [
  ...hostCorePluginSource.matchAll(/"renderer\/index\.html",\s*br#"(.*?)"#,/gs),
].map((match) => match[1]);

// dcode ships its own panels under resources/plugins/, and the host-core crate
// ships the marketplace ones. Both are checked in, so both are read from this
...
test("dcode's shipped plugin panels follow the host chrome contract", () => {
  ...
  for (const panelSource of shippedPanelSources) {
    ...
    // The preload keeps the titlebar band for the host capsule; a panel must
    // never claim the top edge itself or its content slides under the controls.
    assert.doesNotMatch(panelSource, /top:\s*0/);
  }
});

test("the host-core bundled plugin panels follow the host chrome contract", { skip }, () => {
  assert.equal(bundledPanelSources.length, 2);
  for (const panelSource of bundledPanelSources) {
    assert.match(panelSource, /meta name="pi-plugin-chrome" content="v2"/);
    // The host-core panels ship dcode's product name and derive their own
    // offset from the published titlebar variable, so the drag-band contract
    // is asserted in full for them.
    assert.match(
      panelSource,
      /dcode reserves exactly a transparent 46px drag band/,
    );
