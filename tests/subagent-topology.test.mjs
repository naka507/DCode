...
} = await import("../src/renderer/lib/subagent-topology.ts");
...
test("localizeAgentName maps preset names accurately according to locale", async () => {
  const { localizeAgentName } = await import(
    "../src/renderer/features/chat/transcript/model.ts"
  );
  // Chinese locale mappings
  ...
  assert.equal(localizeAgentName("test", true), "测试");
  assert.equal(localizeAgentName("explorer", true), "探索");
  assert.equal(localizeAgentName("explore", true), "探索");
  assert.equal(localizeAgentName("fixer", true), "修复");
  assert.equal(localizeAgentName("ui-designer", true), "设计");
  assert.equal(localizeAgentName("审查", true), "审查");
  assert.equal(localizeAgentName("测试", true), "测试");

  // English locale mappings
  assert.equal(localizeAgentName("code-reviewer", false), "Review");
  assert.equal(localizeAgentName("test-runner", false), "Test");
  assert.equal(localizeAgentName("explorer", false), "Explore");
