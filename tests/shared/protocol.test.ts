...
describe("Plan protocol contracts", () => {
  it("uses protocol v11/schema v16 and exposes the plan, schedule, and shell channels", () => {
    ...
    expect(IPC.invoke.commandShellList).toBe("dcode/commandShell/list");
    ...
    expect(IPC.invoke.providersRefreshModelCatalog).toBe(
      "dcode/providers/refreshModelCatalog",
    );
    ...
    expect(IPC.invoke.fsOpen).toBe("dcode/fs/open");
    expect(IPC_WHITELIST.has(IPC.invoke.fsOpen)).toBe(true);
    expect(IPC.invoke.statsGetTokenUsageHistory).toBe(
      "dcode/stats/getTokenUsageHistory",
    );
    expect(IPC_WHITELIST.has(IPC.invoke.statsGetTokenUsageHistory)).toBe(true);
    expect(IPC.invoke.fsReadImageDataUrl).toBe("dcode/fs/readImageDataUrl");
    expect(IPC_WHITELIST.has(IPC.invoke.fsReadImageDataUrl)).toBe(true);
    expect(IPC.invoke.networkProxyTest).toBe("dcode/network/testProxy");
    expect(IPC_WHITELIST.has(IPC.invoke.networkProxyTest)).toBe(true);
    expect(IPC.invoke.modelConfigImportScan).toBe("dcode/modelConfig/importScan");
    expect(IPC.invoke.modelConfigImportRun).toBe("dcode/modelConfig/importRun");
    ...
    expect(IPC.invoke.projectClone).toBe("dcode/project/clone");
    expect(IPC_WHITELIST.has(IPC.invoke.projectClone)).toBe(true);
    expect(IPC.invoke.speechTranscribe).toBe("dcode/speech/transcribe");
  ...
  it("keeps the artifact and queued execution wire shapes explicit", () => {
    const artifact: PlanArtifact = {
      relativePath: ".dcode/plan/plan.md",
      sha256: "sha256",
      sizeBytes: 6,
    };
