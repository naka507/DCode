/**
 * The SSH bootstrap orchestrator and the descriptor it persists.
 *
 * The suite is portable as written: the transport is a fake injected through
 * `buildTransport`, the checksum comes from an injected `fetchChecksum`, and
 * pairing is an injected `exchangePairing`. No process is spawned and no
 * network is touched, so it runs on every platform — which matters, because
 * this is the ordering contract (probe → resolve → checksum → install →
 * forward → pair) that a refactor can silently scramble.
 */
...
const { createSshBootstrap, synthesizeSshHostKey } = await import(
  "../src/main/remote/ssh-bootstrap.ts"
);
...
// Pinned origin: the bootstrap's artifact URL is derived from it, so this
// suite never reads the checkout's own `package.json`.
const ORIGIN = {
  host: "example.test",
  path: "acme/dcode",
  htmlUrl: "https://example.test/acme/dcode",
};
const ARTIFACT_URL = `${ORIGIN.htmlUrl}/releases/download/v${VERSION}/${ARTIFACT_NAME}`;
...
function harness({ transport = fakeTransport(), checksum, ports = [RESERVED_PORT], exchange } = {}) {
  ...
  const bootstrap = createSshBootstrap({
    version: VERSION,
    // Pinned so the suite never depends on this checkout's configured
    // repository, and so the expected artifact URL below stays literal.
...
test("synthesizeSshHostKey folds an IPv6 literal and is stable", () => {
  ...
  // split the routing id, so the address uses dashes instead.
