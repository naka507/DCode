/**
 * npm install recovery contract.
 *
 * The suite drives a real `pluginImportExtension` handler with stubbed native
 * dialogs and a stubbed dependency runner: a missing npm must lead to picker →
 * validate → remember → retry the same import → register once, and every cancel
 * or ordinary failure must leave the import registered without looping the user.
 *
 * Layout notes:
 *   - The modules live under `src/main/`.
 *   - This file registers the TS hooks itself, so it stays runnable on its own
 *     via `node --test tests/npm-install-recovery.test.mjs`.
 */
...
const { IPC } = await import("@dcode/shared");
const { catalogs } = await import("@dcode/i18n");
const { registerAgentExtensionIpc } = await import("../src/main/agent-extensions-ipc.ts");
const { installExtensionDependencies } = await import("../src/main/npm-installer.ts");
const { readNpmPath, writeNpmPath } = await import("../src/main/npm-preferences.ts");
