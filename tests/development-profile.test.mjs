...
const {
  ...
  resolveDataDir,
} = await import("../src/main/data-paths.ts");
...
test("a development build owns a different data directory than the shipped app", () => {
  const home = join(tmpdir(), "dcode-profile-home");
...
test("DCODE_DATA_DIR still overrides either profile", () => {
  const home = join(tmpdir(), "dcode-profile-home");
  const override = join(tmpdir(), "dcode-explicit-profile");
  ...
  // keeps `DCODE_DATA_DIR=` from naming a relative directory.
  assert.equal(
...
test("an explicit data directory reaches the child processes as an absolute path", () => {
  ...
  // two different trees.
  const relative = join("dcode-relative-profile");
  assert.equal(
    resolveDataDir({
      override: relative,
      development: false,
      home: join(tmpdir(), "dcode-profile-home"),
    }),
    resolve(relative),
  );
});
...
test("a development build takes its own userData before the single-instance lock", async () => {
  ...
  const development = indexSource.search(
    /const isDevelopmentBuild =\s*\n?\s*process\.env\.DCODE_DEV === "1" \|\| !app\.isPackaged;/,
  );
...
test("main resolves one data directory and publishes it to everything below", () => {
  ...
  assert.match(indexSource, /process\.env\.DCODE_DATA_DIR = dataDir;/);
  assert.doesNotMatch(indexSource, /join\(homedir\(\), "\.dcode"\)/);
  ...
  const lockVerdict = indexSource.indexOf(
    "const singleInstanceRequired = !process.env.DCODE_DATA_DIR;",
  );
  assert.ok(lockVerdict > 0);
  assert.ok(
    indexSource.indexOf("process.env.DCODE_DATA_DIR = dataDir;") > lockVerdict,
  );
});
...
test("downstream data directories follow the profile instead of the shipped default", async () => {
  ...
  assert.doesNotMatch(runtimeSource, /join\(homedir\(\), "\.dcode"\)/);
  ...
  assert.doesNotMatch(servicesSource, /join\(homedir\(\), "\.dcode"\)/);
});
