...
/**
 * Work-panel tab state machine.
 *
 * The state this tree lands on is `3173db52`: `58b301a1` removed the
 * `shouldOpenReviewArtifact` case, `77767f73` added the host-chosen-file
 * preference.
 *
 * Retranslations:
 *
 *  - the module under test is `../src/renderer/lib/work-panel-tabs.ts`
 *    instead of `../src/lib/work-panel-tabs.ts`; the module itself is
 *    framework-free, so nothing else in this file changed.
 *  - the `review artifacts are recognized independently of the visible
 *    session` case is gone, because its subject
 *    (`shouldOpenReviewArtifact`) no longer exists: a tool result may not open
 *    or activate a tab (D451). The removal is asserted as an absence in
 *    `work-panel.test.mjs` and `plan-approval-settings.test.mjs`.
 */
...
const {
  ...
  toolWorkPanelTab,
} = await import("../src/renderer/lib/work-panel-tabs.ts");
