...
function pluginsSection(source) {
  const start = source.indexOf("/* ---- Plugins installed index / permission review ----");
  assert.ok(start >= 0, "plugins installed-index section missing");
...
test("plugins page styles use design tokens in both themes", () => {
  ...
  // No blue-slate hardcodes / non-ds fallbacks.
  for (const bad of [
    "#4f7cff",
    "#2a3144",
    "#9aa6bf",
  ...
  // D296: the header and title glyph carry tone, not rules.
...
test("extensions page draws no in-flow dividers", async () => {
  const pluginsCss = await readFile(new URL("../src/renderer/styles/plugins.css", import.meta.url), "utf8");
...
// destination page with controls in that corner (the header actions), so they
// must clear the band.
test("plugins page keeps its header clear of the titlebar band on every platform", () => {
  ...
  for (const selector of ["\\.plugins-header-actions"]) {
    assert.match(
      section,
