...
test("theme changes synchronize the native non-macOS window background", () => {
  assert.match(
    protocolSource,
    /windowSetBackgroundColor:\s*"dcode\/window\/setBackgroundColor"/,
  );
  assert.match(
    apiSource,
...
test("the built-in window palette is declared once", async () => {
  const builtinTheme = await readFile(
    new URL("../src/shared/theme.ts", import.meta.url),
    "utf8",
  );
  ...
  for (const relative of [
    ...
    "../src/main/plugin-panel-host.ts",
    "../src/main/ipc/window-ipc.ts",
    "../src/preload/plugin-panel.ts",
  ]) {
