/**
 * SSH port forwards: argv shape, reuse, teardown, adoption, and refusal.
 *
 * The suite is portable as written: every transport is a fake injected through
 * `buildTransport`, so no `ssh` process is spawned and no POSIX shell is
 * needed. The one thing it does exercise for real is the manager's bookkeeping,
 * which is exactly where a tunnel leak would hide.
 *
 * The suite is portable as written: every transport is a fake injected through
 * `buildTransport`, so no `ssh` process is spawned and no POSIX shell is
 * needed. The one thing it does exercise for real is the manager's bookkeeping,
...
const { createSshTunnelManager, racpUrlForLocalPort, sshTargetOf } = await import(
  "../src/main/remote/ssh-tunnel.ts"
);
const { sshCommonArgs } = await import("../src/main/remote/ssh-transport.ts");
