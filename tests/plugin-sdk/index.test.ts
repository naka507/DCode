...
describe("validateManifest", () => {
  ...
  it("falls back to English for shell locales without a plugin translation", () => {
    ...
    // ADR 0182: Traditional Chinese reads the English entry rather than half a
    // `zh-CN` guess, and any non-Chinese locale does the same.
    ...
    expect(resolvePluginLocalizedString(value, "fr-CA")).toBe("History");
  ...
  it("accepts author as a string or contact object plus homepage/repository", () => {
    expect(validateManifest({ ...base, author: "dcode" }).ok).toBe(true);
    expect(
      validateManifest({
        ...base,
