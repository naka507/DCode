/**
 * The SSH transport itself: argv shape, stream capture, timeouts, and forward
 * lifetimes.
 *
 * The fixture-driven cases carry `{ skip: process.platform === "win32" }`,
 * because they drive the module with `#!/bin/sh` fixture executables that it
 * `chmod`s to `0755`, which Windows cannot execute. That is the same pattern
 * this tree already uses for its macOS-only and symlink-privilege contracts.
 * The two cases that need no fixture (`reserveLocalPort`, `assertSshArgument`)
 * run everywhere, so the portable half of the module is still guarded on this
 * host.
 */
...
const { assertSshArgument, createSystemSshTransport, reserveLocalPort } = await import(
  "../src/main/remote/ssh-transport.ts"
);

/** Every test is bounded well below the module's own 30 s / 120 s defaults. */
const TEST_TIMEOUT_MS = 20_000;
/** The fixture cases need a POSIX shell to execute their `#!/bin/sh` scripts. */
const NEEDS_POSIX = {
  timeout: TEST_TIMEOUT_MS,
  skip:
    process.platform === "win32"
      ? "the module spawns `#!/bin/sh` fixtures with `shell: false`, which Windows cannot execute (ENOENT)"
      : false,
};
...
async function writeFixture(t, body, name) {
  const dir = await mkdtemp(join(tmpdir(), "dcode-ssh-fixture-"));
...
test("exec passes the command as one argv entry behind the common options", NEEDS_POSIX, async (t) => {
...
test("execWithInput runs the script through `sh -s` with the body on stdin", NEEDS_POSIX, async (t) => {
...
test("a non-zero exit becomes a typed error carrying both streams", NEEDS_POSIX, async (t) => {
...
test("the stdout of that error is redacted of a live pairing token", NEEDS_POSIX, async (t) => {
...
test("a command outliving timeoutMs is killed, not awaited", NEEDS_POSIX, async (t) => {
...
test("a binary that does not exist rejects instead of crashing", NEEDS_POSIX, async (t) => {
  const binary = await writeFixture(t, FIXTURES.argvEcho, "ssh-argv-echo");
...
test("forward rejects fast, with the ssh diagnostic, when ssh exits instead", NEEDS_POSIX, async (t) => {
...
test("forward resolves once the port answers and close() is idempotent", NEEDS_POSIX, async (t) => {
...
test("dispose reaps a live forward and leaves a finished exec alone", NEEDS_POSIX, async (t) => {
