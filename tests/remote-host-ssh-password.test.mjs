 ...
 *
 * Platform note: the fixture-driven cases write `#!/bin/sh` scripts with
 * `0700` modes, which Windows cannot execute, so they carry
 * `{ skip: process.platform === "win32" }` — the same pattern this tree already
 * uses elsewhere. The argv cases, the `assertSshPassword` cases, and the
 * registry cases need no fixture and run on every platform, which keeps the
 * password-record contract guarded here.
 */
...
test("a password target reaches ssh through the askpass helper and nowhere else", NEEDS_POSIX, async (t) => {
...
test("a key-authenticated transport is handed no askpass material", NEEDS_POSIX, async (t) => {
...
test("each command gets its own credential, and none outlives it", NEEDS_POSIX, async (t) => {
...
test("dispose removes credential material that no child has reclaimed", NEEDS_POSIX, async (t) => {
...
test("the helper script is POSIX shell that reads the secret by path", () => {
...
test("createSshAskpass writes a 0600 secret in a 0700 directory and cleans up", NEEDS_POSIX, async (t) => {
