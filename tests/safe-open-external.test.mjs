...
} = await import("../src/main/safe-open-external.ts");
...
const runtimeSource = read("../src/main/plugin-runtime.ts");
const viewHostSource = read("../src/main/plugin-view-host.ts");
const browserSource = read("../src/main/browser-view.ts");
const updaterSource = read("../src/main/updater.ts");

test("parseAllowedExternalUrl permits http, https, and mailto", () => {
  assert.equal(
    parseAllowedExternalUrl("https://github.com/example/dcode"),
    "https://github.com/example/dcode",
  );
...
test("main, plugins, preview, and updater share the allowlist before openExternal", () => {
  ...
  // The updater validates the releases URL through the same allowlist helper
  // every other external open uses. The URL itself comes from the configured
  // repository, so the call must read the resolved state instead of a vendor
  // literal, and a build with no configured origin must fail closed rather than
  // open another project's releases page.
  assert.match(
    updaterSource,
    /const url = parseAllowedExternalUrl\(this\.state\.releasesUrl\)/,
  );
  assert.match(updaterSource, /if \(!url\) throw new Error\("DISALLOWED_EXTERNAL_URL"\)/);
  assert.doesNotMatch(updaterSource, /parseAllowedExternalUrl\([^)]*RELEASES_URL/);
});
