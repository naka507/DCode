...
- 🖥️ **工作面板原生扩展插件体系**：采用简洁单单词命名的内置面板生态（`browser`、`files`、`terminal`、`documents`）：
  - **`terminal` 终端面板**：集成交互式 Shell 终端，跨平台自动探测平台默认 Shell（Windows 优先使用 PowerShell/CMD，macOS/Linux 优先使用 Bash/Zsh），提供快捷指令卡片、历史命令上下键回溯及智能体工具直连执行能力。
  - **`documents` 文档面板**：集成丰富文档阅读与大纲导航面板，支持 Markdown 阅读时长与字数统计、分级大纲目录锚点跳转、源码与预览切换，并原生支持 PDF 渲染浏览。
