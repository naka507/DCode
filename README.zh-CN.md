...
- 📊 **上下文容量细分与精准洞察 (Context Capacity Inspector)**：输入框集成实时上下文容量监控环与来源细分面板。精确统计系统提示词（System Prompt）、系统/扩展工具定义（Tools）、以及对话历史（Messages）的真实 Token 占用比例与缓存命中率，多语言环境精准修正，彻底消除指标缺省。
