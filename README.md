...
- **Zero-Overhead Engineering Skills Catalog**: Bundled engineering skills (`dcode/vue-best-practices`, `dcode/react-best-practices`, `dcode/agent-browser`, `dcode/dogfood`, `dcode/electron`) loaded on-demand via the agent `Skill` tool, keeping idle token consumption and memory usage at absolute zero.
