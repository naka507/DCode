...
The two paths no longer ship the same bytes: this copy carries the repository's
product rename as well (see local changes below), while the published artifact
is the untouched upstream build.
...
The files below started as byte-identical copies of that commit. Four of the
five now carry local edits, listed under local changes below, so the checksum
table is the *upstream* record, not a description of the bytes in this
directory. Only `views/index.html` is still byte-identical. Line endings are
LF: the upstream commit stores LF, and this repository's `.gitattributes` keeps
it that way.
...
- `manifest.json` gains `"license": "MIT"` (after `author`). Every shipped
  plugin carries its license, and the upstream manifest predates that
  convention.
- Every user-visible occurrence of the product name is renamed to `dcode`
  (`main.js`'s header comment, `README.md`'s prose, `manifest.json`'s
  `engines` key). `engines.piDesktop` is the declared host-version field, and
  this repository's SDK types it as `engines: { dcode?: string }`
  (`src/plugin/sdk/index.ts`); the rename keeps the manifest's declared host in
  step with the name the SDK documents. (Nothing enforces the field yet, in
  either tree — it is a declaration, not a gate.) The rename touches
  `views/assets/index.js` too: that is the built bundle, and the string is
  inside it.
...
3. Re-apply the local changes above: `"license": "MIT"` in `manifest.json`,
   re-create `package.json` (the marker is local and the tag does not carry
   it), and rename the product name in the four files that spell it.
4. Update `Origin`, the checksum table and the release section here. Nothing in
   this fold pins the commit hash or the checksums, so the table is the only
   record; the upstream test that used to consume it
   (`test/bundled-plugins.test.mjs`) lives in the upstream checkout, not here.
