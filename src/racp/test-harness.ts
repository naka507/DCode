import type { AskToolResolution } from "@dcode/shared";
import { RacpError, AgentHost, type ApprovalPort, type Principal, type RuntimePort, type SessionPort, type SessionSummary, type TurnStartRequest } from "@dcode/agent-host";
...
export function buildHost(limits: Partial<import("@dcode/shared").RacpLimits> = {}): { host: AgentHost; runtime: FakeRuntime; sessions: Map<string, SessionSummary>; approvals: { tool: Array<{ requestId: string; decision: string }> } } {
...
export type Harness = {
  ...
  connect(token: string, options?: Partial<ConstructorParameters<typeof RacpClient>[0]>): Promise<{ client: RacpClient; events: import("@dcode/shared").RacpEventEnvelope[]; link: () => MemoryLink }>;
};

export async function harness(options: { limits?: Partial<import("@dcode/shared").RacpLimits>; operations?: Partial<RacpHostOperations> } = {}): Promise<Harness> {
  ...
  return {
    ...
    async connect(token, clientOptions = {}) {
      const events: import("@dcode/shared").RacpEventEnvelope[] = [];
