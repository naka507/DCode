...
import type { SessionMessageOrigin, TrustedExtensionSpec } from "@dcode/shared";
...
function testRuntimeIdentity(sessionId: string) {
  if (process.env.DCODE_PLAN_UI_PROBE !== "1") {
...
const bootProxy = process.env.DCODE_PROXY_JSON;
if (bootProxy) {
  try {
