 ...
 * of real tool calls. dcode has no such tool, so the whole batch lands as
...
function resolveCompactionStrategy(
  ...
  return process.env.DCODE_COMPACTION_STRATEGY === "fresh_window"
    ? "fresh_window"
    : "summary";
}
...
export type AgentRuntimeOptions = {
   ...
   * `DCODE_COMPACTION_STRATEGY` and otherwise summarizes.
   */
...
- Never delegate what you can finish in a couple of tool calls, and never delegate anything that needs the user.`,
      ...
      // `multi_tool_use.parallel` wrapper as assistant text. dcode has no
    ...
    while (
      ...
      try {
        if (!this.pendingSteering.size) {
          await this.waitForDelegations(targets, 1, null, waitAbort.signal);
        }
      } finally {
      ...
      if (settled.length === 0) {
        if (this.pendingCurrentTurnDelegations().length > 0) continue;
        return;
      }
      ...
      parameters: Type.Object({
        ...
        mode: Type.Optional(
          Type.Union([Type.Literal("all"), Type.Literal("any")], {
            description: "Wait for the first to finish (any, default) or every target (all).",
          }),
        ),
      ...
      execute: async (toolCallId, params, signal) => {
        const ids =
          isRecord(params) && Array.isArray(params.delegationIds)
            ...
            : [];
        const mode = isRecord(params) && params.mode === "all" ? "all" : "any";
        const minCompleted =
  ...
  private claimProviderRetry(
    ...
    // Request setup and stream delivery share this counter. A gateway that fails
