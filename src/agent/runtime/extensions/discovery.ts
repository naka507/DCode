...
export type DiscoverTrustedExtensionsInput = {
  ...
  agentDir: string;
  /** Workspace root; project extensions live under `<workspace>/.dcode/extensions` (or `.pi/extensions`). */
  projectPath?: string;
...
export function discoverTrustedExtensions(
  ...
  if (input.projectPath) {
    add(discoverExtensionsInDir(join(input.projectPath, ".dcode", "extensions"), "project"));
