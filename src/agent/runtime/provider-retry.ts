...
/**
 * Retries allowed after the first non-rate-limit transient failure. The earlier implementation
 ...
 * `Retry-After` wins outright, so the 502/503 burst clears by waiting
...
/**
 * `DCODE_STREAM_IDLE_TIMEOUT_MS` overrides the zero-event idle budget;
...
export function streamIdleTimeoutMs(): number {
  const raw = process.env.DCODE_STREAM_IDLE_TIMEOUT_MS;
