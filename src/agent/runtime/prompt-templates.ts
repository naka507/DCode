 ...
 * `.pi/prompts` assets behave identically in pi CLI and dcode.
 *
 * Discovery: `<workspace>/.dcode/prompts/*.md` (project, with `.pi/prompts` fallback) and
...
/**
 * Prompt-enhancement templates live in `@dcode/shared` so the runtime,
 * the settings UI, and the "restore default" action all read one copy
 * (`src/shared/prompt-enhancement.ts`).
 */
...
export function composerTemplateDirs(
  ...
  const dcodePrompts = join(workspaceRoot, ".dcode", "prompts");
  const piPrompts = join(workspaceRoot, ".pi", "prompts");
  const project = existsSync(dcodePrompts)
    ? dcodePrompts
    : existsSync(piPrompts)
      ? piPrompts
      : dcodePrompts;
  return {
    project,
...
export async function loadComposerTemplates(
  ...
  const projectPaths = dirs.project ? [dirs.project] : [];
  if (!overrideDirs && workspaceRoot) {
    const legacyPiPrompts = join(workspaceRoot, ".pi", "prompts");
    if (!projectPaths.includes(legacyPiPrompts) && existsSync(legacyPiPrompts)) {
      projectPaths.push(legacyPiPrompts);
    }
  }
  ...
  for (const { promptTemplate, source } of loaded.promptTemplates) {
    ...
    if (source === "project") {
      for (const p of projectPaths) {
        argumentHint = await readArgumentHint(join(p, `${name}.md`));
        if (argumentHint) break;
      }
    } else {
      argumentHint = await readArgumentHint(join(dirs.user, `${name}.md`));
    }
    byName.set(name, {
      ...promptTemplate,
      name,
