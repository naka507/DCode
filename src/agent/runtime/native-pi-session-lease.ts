...
/**
 * Cooperative dcode lease plus optimistic file validation.
...
export class NativePiSessionLease {
  ...
  static canAcquire(path: string, current: NativePiSnapshot): boolean {
    try {
      const previous = JSON.parse(readFileSync(`${path}.dcode.lock`, "utf8")) as LeaseRecord;
