import type { ProjectWorkspace } from "@dcode/shared";
import {
  sessionIsArchived,
  sessionIsPinned,
  sortSessions,
...
/**
 * The session-presentation algebra lives in `src/shared` so the main process
 * can build the tray's session rows with exactly the order the sidebar shows.
 * This module keeps the renderer-only additions — `manualTitle`, project
 * metadata, and the persisted preference record — and re-exports the shared
 * half unchanged.
 */
export { sessionIsArchived, sessionIsPinned, sortSessions };
...
/** `manualTitle` is renderer-owned; pinned/archived/order come from shared. */
export type SessionMeta = SessionPresentationMeta & {
...
/**
 * Clamp a sidebar width to the expanded range. `max` is the live three-column
 * budget: a squeezed window caps the column below `SIDEBAR_WIDTH_MAX` so a
 * user-chosen width can never cross MainChat's 450px floor.
 */
