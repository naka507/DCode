...
import { toIpcPayload } from "./ipc-payload";
...
// `src/main/importers/agent-*-scan.ts`.
...
async function invoke<T>(channel: string, ...args: unknown[]): Promise<T> {
  ...
  // a `ref` array handed over as-is fails with Chromium's own "could not be
  // cloned" before any main-process handler runs. See `lib/ipc-payload.ts`.
  const payload = args.map(toIpcPayload);
  const result = await window.dcode.invoke<T>(channel, ...payload);
  if (!result.ok) {
  ...
  return {
    ...
    defaultCommandShell: isCommandShellId(
      ...
      ? (settings as { defaultCommandShell: AppSettings["defaultCommandShell"] })
          .defaultCommandShell
      : defaultCommandShellForPlatform(window.dcode?.platform ?? ""),
...
export const api = {
  ...
  /**
   * Report that the catalog is snapshot-only.
   *
   * The bundled snapshot is the sole data source, so this no longer fetches
   * anything; it keeps the channel so an older renderer still gets a truthful
  ...
  getDroppedFilePath: (file: File) =>
    window.dcode?.getDroppedFilePath?.(file) ?? null,
  pickPhotos: () =>
  ...
  getTokenUsageHistory: (query?: { startDate?: number; endDate?: number; bucket?: "day" | "week" | "month" }) =>
    invoke<import("@dcode/shared").TokenUsageHistoryResult>(
  ...
  onTraySessionActivated: (listener: (sessionId: string | null) => void) => {
    if (!window.dcode?.on) return () => undefined;
    return window.dcode.on(IPC.event.traySessionActivated, (payload) => {
  ...
  onWindowMaximized: (listener: (event: { maximized: boolean }) => void) => {
    if (!window.dcode?.on) return () => undefined;
    return window.dcode.on(IPC.event.windowMaximized, (payload) =>
      listener(payload as { maximized: boolean }),
    );
  },
  onWindowFullScreen: (listener: (event: { fullScreen: boolean }) => void) => {
    if (!window.dcode?.on) return () => undefined;
    return window.dcode.on(IPC.event.windowFullScreen, (payload) =>
  ...
  onWorkPanelResize: (
    ...
    if (!window.dcode?.on) return () => undefined;
    return window.dcode.on(IPC.event.windowWorkPanelResize, (payload) =>
      listener(
        payload as {
          phase: "preview" | "commit";
          panelWidth: number;
  ...
  onMenuCommand: (listener: (command: AppMenuCommand) => void) => {
    if (!window.dcode?.on) return () => undefined;
    return window.dcode.on(IPC.event.menuCommand, (payload) =>
      listener((payload as { command: AppMenuCommand }).command),
    );
  },
  onBrowserState: (listener: (state: BrowserState) => void) => {
    if (!window.dcode?.on) return () => undefined;
    return window.dcode.on(IPC.event.browserState, (payload) =>
  ...
  onNotificationChanged: (
    ...
    if (!window.dcode?.on) return () => undefined;
    return window.dcode.on(IPC.event.notificationChanged, (payload) =>
  ...
  onSessionsChanged: (
    ...
    if (!window.dcode?.on) return () => undefined;
    return window.dcode.on(IPC.event.sessionsChanged, (payload) =>
      listener(
        (payload ?? {}) as {
          reason?: string;
          pluginId?: string;
