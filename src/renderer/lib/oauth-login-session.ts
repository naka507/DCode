 ...
 * handler rather than from a mount effect: a double-invoked effect would open a
 * second browser and fight the first one for the local callback port. The
 * session therefore keeps every event it has delivered and replays it to
 * whoever subscribes, so a dialog that mounts, unmounts and mounts again still
