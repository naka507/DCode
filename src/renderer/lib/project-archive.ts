/**
 * Framework-free half of the projects archive.
 *
 * The framework-free half of the projects archive: the path predicate, the section
 * tables, the relative-time formatter, the two query predicates, the index
 * build and the row algebra are plain functions and tables, so they live here
 * and `pages/ProjectsPage.vue` is only the component that consumes them.
 *
 * This module is the single definition of all of those, and the page imports it.
 *
 * Type-level adaptations:
 *  - `ProjectMeta` comes from `./sidebar-preferences` (the same module
 *    that declares it, so the path is unchanged).
 *  - `normalizeProjectPath` is taken from `./sidebar-preferences` rather than
 *    `./sidebar-session-groups`, because that is the module that names it.
 *    The two are the same function; `sidebar-session-groups` re-exports this
 *    one.
 *  - `projectRowId` is exported here rather than beside the index component, because
 *    a `<script setup>` block cannot carry a value export, so the pure helper
 *    lives in the framework-free half and the SFC imports it — the export
 *    surface the page needs (`projectRowId`) is unchanged.
 *  - `ProjectIndexItem` keeps the exact shape (`archived?`), so the
 *    types `SORT_MODES`, `ProjectDialogTarget`, `ProjectEditTarget` and
 *    `ProjectDeleteTarget` are appended at the bottom for the page's dialogs.
 */

import type { ProjectMeta } from "./sidebar-preferences";
import { normalizeProjectPath } from "./sidebar-preferences";
import { projectColor, type RecentProject } from "./recent-projects";
...
/* ---------- Fold-only additions, kept from `pages/projects-page.ts` ---------- */

/** The toolbar's mode table, in the order the segmented control renders it. */
export const SORT_MODES: SortMode[] = ["recent", "name"];

/** Target of the project dialogs that only need the group identity. */
...
/** Target of the edit dialog, which also derives from the group's roots. */
export type ProjectEditTarget = ProjectDialogTarget & {
  roots?: ProjectRoot[];
};

/**
 * Target of the delete dialog. `roots` rides along so the page can keep
 * deriving the project's live running sessions while the dialog is open,
 * instead of a snapshot taken on click.
 */
