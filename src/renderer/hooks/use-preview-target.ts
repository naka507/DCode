 ...
 *
 * `useCallback` becomes a plain function, and `useMemo` a computed.
...
export function useOpenPreviewTarget() {
  ...
  return (target: ChatPreviewTarget) =>
...
export function useOpenChatFileRef() {
  ...
  const fileViewAvailable = computed(() =>
    hasPluginView(store.appState?.pluginViews ?? [], FILE_MANAGER_PLUGIN_TAB),
  );

  return (path: string, baseDir?: string, mimeType?: string) => {
    ...
    void (async () => {
      ...
      if (!match) {
        state.showToast(t("chat.fileRefMissing", { name: raw }), {
          variant: "error",
        });
        return;
      }
      if (match.root === "workspace") {
        ...
        if (inPrimary && isHtmlFilePath(match.relativePath)) {
          state.openUrlInWorkPanel(match.relativePath);
          return;
        }
        if (fileViewAvailable.value) {
          state.openWorkPanelTab(fileManagerPluginTab(target));
          return;
        }
        state.openFileInWorkPanel(target, mimeType);
        return;
      }
      state.openFileInWorkPanel(match.absolutePath, mimeType);
    })();
  };
}
