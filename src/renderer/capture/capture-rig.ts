 ...
 * `DCODE_CAPTURE=1`). Nothing here ships on the production code path:
  ...
  const methods: CaptureRigMethods = {
    openWorkPanelArtifact: (
      ...
      if (kind === "file" && resource) {
        appState().openFileInWorkPanel(resource);
        return;
      }
      if (kind === "browser") {
        appState().openWorkPanelTab(browserPluginTab(resource));
        return;
      }
      if (kind === "review") {
        appState().openWorkPanelTab(toolWorkPanelTab("review"));
      }
    },
    collapseWorkPanel: () => {
      if (!window.__PI_CAPTURE__) return;
      appState().collapseWorkPanel();
    },
    /**
    ...
    openWorkPanel: () => {
      if (!window.__PI_CAPTURE__) return;
      appState().openWorkPanel();
    },
    openNewWorkPanelTab: () => {
      if (!window.__PI_CAPTURE__) return;
      appState().openNewWorkPanelTab();
    },
    setWorkPanelWidth: (width) => {
      if (!window.__PI_CAPTURE__) return;
      appState().setWorkPanelWidth(width);
    },
    seedTranscript: (count = 12) => {
      ...
      const samples: [role: "user" | "assistant", content: string][] = [
        ...
        [
          "assistant",
          "这是因为 Rust 侧还没编译。运行 `cargo build -p host-core`，产物会出现在 `target/debug/` 下，Electron 主进程会自动拾取。",
        ],
        ["user", "编译通过了，界面也起来了"],
        [
          "assistant",
    ...
    seedReviewChanges: (count = 4) => {
      ...
      const samples = [
        {
          tool: "Edit",
          path: "src/renderer/components/workpanel/ReviewTab.vue",
          operation: "edit",
          status: "modified",
          state: "active",
          additions: 4,
        ...
        {
          tool: "Write",
          path: "src/renderer/lib/review-row-metrics.ts",
          operation: "write",
          status: "added",
          state: "active",
          additions: 12,
        ...
        {
          tool: "Edit",
          path: "src/renderer/styles/work-panel.css",
          operation: "edit",
          status: "deleted",
          state: "active",
          additions: 0,
        ...
        {
          tool: "Edit",
          path: "src/renderer/styles/messages.css",
          operation: "edit",
          status: "modified",
          state: "rolledBack",
          additions: 3,
    ...
    seedRunRows: (count = 3) => {
      ...
      const samples = [
        ...
        {
          command: "pnpm --filter @dcode/desktop build",
    ...
    seedDelegationRows: (count = 3) => {
      ...
      const messages = [
        ...
        delegateRow(0, "code-reviewer", "src/renderer/stores/app-store.ts"),
        {
        ...
        delegateRow(1, "code-reviewer", "src/agent/runtime/runtime.ts"),
    ...
    seedSidebarStatuses: () => {
      ...
      const sessions = appState().sessions.slice(0, 4);
    ...
    ensureVisualFixtures: async () => {
      ...
      try {
        ...
        const existing = new Set(
          (appState().sessions || []).map((s) => (s.title || "").trim()),
        );
        const titles = [
          "同步代码",
          "你好",
        ...
        for (const title of titles) {
          ...
          if ((appState().sessions?.length ?? 0) >= 14) break;
          await api.createSession({ title });
          existing.add(title);
        }
        await appState().refreshSessions();
        const preferred = appState()
        ...
        if (preferred) {
          ...
          await appState().selectSession(preferred.id);
        }
      } catch {
