 ...
 *
 * The `subagent-models` module. It is framework-free, so the only
 * difference from the rest of the tree is the import path of
 * `defaultModelOptions`, which lives at `./default-model`.
 */
import { modelIdsMatch, type ProviderPublic } from "@dcode/shared";
...
export function pinMatchesChoice(pin: string, choice: SubagentModelChoice): boolean {
  ...
  return providerAlias(choice.providerName) === alias;
}
...
export function subagentModelChoices(
  ...
  for (const { provider, modelId } of defaultModelOptions(runnableProviders)) {
