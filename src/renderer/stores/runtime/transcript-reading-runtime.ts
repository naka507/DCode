/**
 * Transcript reading runtime: search navigation and historical paging.
 *
 * The module imports no view-layer or state-library API;
 * `delegationIdForMessage` comes from `src/renderer/lib/subagent-panel.ts`, and
 * `../app-state` / `../slices/types` are the renderer's type declarations for
 * the shapes.
 */
import type { SessionDetail } from "@dcode/shared";
