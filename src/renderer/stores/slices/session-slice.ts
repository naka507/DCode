/**
 * Session slice: list refresh, selection, plan refresh, new and forked sessions.
 *
 * Translation goes through the vue-i18n instance in `../../i18n`
 * (`i18n.global.t(...)`). Message keys, refresh coalescing, commit order and the
 * `pendingNewSessionRequests` handshake are unchanged.
 */
...
export type SessionSliceDependencies = StoreAccess & {
  ...
  rememberSessionCompactions: (
    ...
    session:
      | {
          compaction?: import("@dcode/shared").ContextCompactionRecord;
          compactions?: import("@dcode/shared").ContextCompactionRecord[];
        }
      | null
      | undefined,
  ) => void;
...
> {
  ...
  return {
      ...
      const alignWorkspace = async (projectPath?: string | null) => {
        if (projectPath) {
          if (
            ...
            if (!workspace) throw new Error(i18n.global.t("errors.workspaceActivationFailed"));
          }
    ...
    forkSession: async (id) => {
      ...
      if (!source) throw new Error(i18n.global.t("errors.sessionNotFound"));
      ...
      const sourceTitle = source.title.trim() || i18n.global.t("chat.untitledTask");
      const result = await api.forkSession(
        id,
        i18n.global.t("nav.branchTitle", { title: sourceTitle }),
      );
