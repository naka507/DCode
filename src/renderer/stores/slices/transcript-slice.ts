/**
 * Transcript slice: compaction, retries, edits, revisions and abort.
 *
 * Translation goes through the vue-i18n instance in `../../i18n`
 * (`i18n.global.t(...)`). Canonical-input hydration, the smart-stop restore path
        ...
        if ((error as { code?: string })?.code !== "CONTEXT_COMPACTION_FAILED") {
          get().showToast(
            error instanceof Error
              ? error.message
              : i18n.global.t("contextCompaction.failed"),
            { variant: "error" },
          );
        }
      }
    ...
    rollbackWorkspaceChange: async (messageId, snapshotId) => {
      ...
      try {
        ...
        if (result.status === "rolledBack" || result.status === "alreadyRolledBack") {
          ...
          get().showToast(i18n.global.t("panel.review.rollbackConflict"), {
            variant: "warning",
          });
        } else if (result.status === "unavailable") {
          get().showToast(i18n.global.t("panel.review.rollbackUnavailable"), {
            variant: "warning",
          });
        }
        return result;
      } catch (error) {
        get().showToast(i18n.global.t("panel.review.rollbackError"), {
          variant: "error",
        });
        set({
          error: error instanceof Error ? error.message : String(error),
