/**
 * Interaction slice: navigation, permission/ask/plan resolution and toasts.
 *
 * Translation goes through the vue-i18n instance in `../../i18n`
 * (`i18n.global.t(...)`). Toast de-duplication, the toast stack limit and the
 * navigation stack are unchanged.
 */
  ...
  return {
    ...
    resolvePlan: async (resolution: PlanResolveRequest) => {
      ...
      if (
        ...
        throw new Error(i18n.global.t("errors.planApprovalUnavailable"));
      }
      const request = (async () => {
        try {
