/**
 * Events slice: host agent events and planning-state changes.
 *
 * Translation goes through the vue-i18n instance in `../../i18n`
 * (`i18n.global.t(...)`). Frame batching, projection order and toast keys are
 * unchanged.
 */
    ...
    handleAgentEvent: (envelope) => {
      ...
      switch (event.type) {
        ...
        case "compaction_end":
          ...
          if (event.ok) {
            get().showToast(i18n.global.t("contextCompaction.longThreadWarning"), {
              variant: "warning",
            });
            if (event.fallback) {
              get().showToast(i18n.global.t("contextCompaction.recovered"), {
                variant: "warning",
              });
            } else if (event.reason === "overflow") {
              get().showToast(i18n.global.t("contextCompaction.retrying"), {
                variant: "warning",
              });
            } else if (event.reason === "manual") {
              get().showToast(i18n.global.t("contextCompaction.completed"), {
                variant: "info",
              });
            }
          } else if (event.reason === "manual") {
            get().showToast(
              event.error?.message || i18n.global.t("contextCompaction.failed"),
              { variant: "error" },
            );
          }
          break;
