/**
 * Project slice: project tabs, sidebar preferences and session lifecycle.
 *
 * Translations come from the vue-i18n instance in `../../i18n`, read through
 * `i18n.global.t(...)`.
 */
...
async function createNamedProjectGroup(
  ...
  if (!normalizedName) {
    throw new Error(i18n.global.t("errors.projectNameLength"));
  }
  ...
  if (uniqueFolders.length === 0) {
    throw new Error(i18n.global.t("project.createFolderRequired"));
  }
...
> {
  return {
    ...
    renameSession: async (id, title) => {
      ...
      if (!nextTitle) throw new Error(i18n.global.t("errors.sessionTitleEmpty"));
      manualSessionTitles.add(id);
      const result = await api.renameSession(id, nextTitle);
      if (!result.ok) throw new Error(i18n.global.t("errors.sessionNotFound"));
      set((state) => ({
        sessionMeta: {
          ...state.sessionMeta,
