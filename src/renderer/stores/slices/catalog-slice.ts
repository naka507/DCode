/**
 * Catalog slice: providers, provider models, plugins and notifications.
 *
 * No framework API is used here (no i18n, no view layer), so the slice stays
 * pure state.
 */
import type {
  AppNotification,
...
> {
  return {
    refreshProviders: async () => {
      ...
      // A newer refresh started while this one was in flight: its answer is the
      // current one, so this stale reply must not overwrite the catalog.
