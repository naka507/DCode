/**
 * Queue slice: prompt submission, steering and the renderer-owned prompt queue.
 *
 * Translation goes through the vue-i18n instance in `../../i18n`
 * (`i18n.global.t(...)`). Acceptance rules, optimistic rows and queue mirroring
 * are unchanged.
 */
  ...
  return {
    ...
    steerPrompt: async (content, draft) => {
      ...
      if (
        ...
        get().showToast(i18n.global.t("chat.steeringUnavailable"), { variant: "info" });
        return false;
      }
      ...
      try {
        ...
        get().showToast(
          failure.code === "TURN_NOT_FOUND"
            ? i18n.global.t("chat.steeringUnavailable")
            : failure.message,
          { variant: "error" },
        );
        return false;
    ...
    sendPrompt: async (content, draft, requestedSessionId) => {
      ...
      try {
        ...
        if (!sessionId) throw new Error(i18n.global.t("errors.noActiveSession"));
        ...
        if (get().runningSessions[sessionId]) {
          ...
          if (
            ...
            get().showToast(i18n.global.t("chat.nativeSessionBusy"), { variant: "info" });
            return false;
          }
