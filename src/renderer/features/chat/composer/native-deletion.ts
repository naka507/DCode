/**
 * Composer deletion guard: keep native editing and undo/redo.
 *
 * The module is framework-free — it talks to the DOM and to `./editor` only.
 * The guard is installed from a `watch` on the editor ref and disposed through
 * `onCleanup` (see `ComposerInput.vue`).
 */
...
export function installComposerDeletionGuard(editor: HTMLElement): () => void {
  ...
  // Native beforeinput includes deletion target ranges; a synthetic
  // beforeinput does not consistently expose deletion events. Capture input
  // before the DOM is serialized into the draft.
