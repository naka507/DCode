 ...
 *
 * The module is framework-free (DOM + `@dcode/shared` only); the only import is
 * `./model`.
 */
import {
  fileReferenceLabel,
...
function buildChipElement(
  ...
  /*
    One activation path for click and keyboard, so a chip that is not a text
    reference (an image) is a plain listitem with no handler (78a073a5).
  */
