 ...
 *
 * Icons are components rather than JSX elements. `h()` is the
 * equivalent, and each icon is wrapped in a zero-prop functional component so
 * the menu can render it with `<component :is>` and keep the builder a plain
 * function of data. The `t` parameter stays a parameter:
 * these are pure builders, not components, so the caller passes the translator
 * it already holds.
 */
...
import type { ContextMenuItem } from "../../../lib/context-menu-state";

/** The translator the builders need; `vue-i18n`'s `t` satisfies it. */
export type MenuTranslate = (key: string, params?: Record<string, unknown>) => string;
...
export function conversationMenuItems({
  ...
  const scrollTo = (top: number) => () => scrollRef.value?.scrollTo({ top });
  return [
    {
    ...
    },
    {
      ...
      icon: selectIcon(),
      separatorBefore: true,
      onSelect: () => actions.selectText(contentRef.value),
    },
    {
      id: "scroll-top",
...
/*
  Icons arrive as zero-prop components rather than elements so a builder stays a
  plain function of data. They are created per call, which Vue reconciles by
  position inside the menu — the same shape the hover toolbar already uses.
*/
function copyIcon(): Component {
  return () => h(IconCopy, { size: 14 });
}

function selectIcon(): Component {
  return () => h(IconTextSelect, { size: 14 });
}

function pencilIcon(): Component {
  return () => h(IconPencil, { size: 14 });
}

function trashIcon(): Component {
  return () => h(IconTrash, { size: 14 });
}

function previousIcon(): Component {
  return () => h(IconChevronLeft, { size: 14 });
}

function nextIcon(): Component {
  return () => h(IconChevronRight, { size: 14 });
}

function regenerateIcon(): Component {
  return () => h(IconReview, { size: 14 });
}

function branchIcon(): Component {
  return () => h(IconBranch, { size: 14 });
}

function upIcon(): Component {
  return () => h(IconArrowUp, { size: 14 });
}

function downIcon(): Component {
  return () => h(IconArrowDown, { size: 14 });
}
