...
import type { FsEntry, FsImageDataUrlResult, FsReadResult } from "@dcode/shared";
...
/**
 * Windows spellings of "absolute" that POSIX already folds into the root when
 * the leading `/` is stripped: a drive root (`C:\x`, `C:/x`, bare `C:`) and a
 * UNC share (`\\server\share\x`). Left in place, `path.resolve` treats them as
 * absolute, returns them as-is, and the containment guard below calls a
 * root-relative path an escape.
 */
const WINDOWS_DRIVE_PREFIX = /^[A-Za-z]:[/\\]*/;
// Neither the host nor the share segment may be a dot segment, so `//../..`
// still reads as an escape instead of being swallowed into the root.
const WINDOWS_UNC_PREFIX =
  /^[/\\]{2,}(?!\.{1,2}(?:[/\\]|$))[^/\\]+[/\\](?!\.{1,2}(?:[/\\]|$))[^/\\]+[/\\]*/;

/**
 * Resolve `rel` inside `root`, rejecting `..` escapes. An absolute input is
 * not trusted as-is: it is read as root-relative (see the test contract).
...
export function resolveWithinRoot(root: string, rel: string): string | null {
  ...
  const cleanRel = String(rel ?? "")
    // UNC first: `/^[/\\]+/` alone would eat the `\\` and leave `server\share`
    // sitting in the relative path.
    .replace(WINDOWS_UNC_PREFIX, "")
