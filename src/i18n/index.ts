 ...
 *
 * Only English and Simplified Chinese ship. Every other language — including
 * Traditional Chinese, which used to have its own catalog — resolves to one of
 * these two, so there is no locale the picker can offer but the catalogs cannot
 * answer.
 */
...
/**
 * Map any OS/browser language tag onto a shipped locale.
 *
 * Every Chinese variant — Traditional included — resolves to Simplified
 * Chinese, which is the only Chinese catalog that ships. That is deliberate:
 * a `zh-TW` system reads Simplified Chinese rather than nothing.
 */
