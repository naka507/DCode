 ...
 * DCODE_PLAN_UI_PROBE=1, it exposes a Main-only global the harness calls
...
const PLAN_UI_PROBE_GLOBAL = "__DCODE_PLAN_UI_PROBE";
...
export function createPlanUiProbe(deps: PlanUiProbeDeps) {
  ...
  function planUiProbeErrorText(error: unknown): string {
    ...
    const secret = process.env.DCODE_TEST_API_KEY;
  ...
  async function planUiProbeLiveSetup(
    ...
    const apiKey = process.env.DCODE_TEST_API_KEY;
    const baseUrl = process.env.DCODE_TEST_BASE_URL;
    const modelId = process.env.DCODE_TEST_MODEL;
    const missing = [
      !apiKey?.trim() ? "DCODE_TEST_API_KEY" : null,
      !baseUrl?.trim() ? "DCODE_TEST_BASE_URL" : null,
      !modelId?.trim() ? "DCODE_TEST_MODEL" : null,
  ...
  function installPlanUiProbe() {
    if (process.env.DCODE_PLAN_UI_PROBE !== "1") return;
