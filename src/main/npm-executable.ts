...
export const defaultDependencyRunner: DependencyCommandRunner = (
  ...
  const isolatedUserConfig = join(tmpdir(), `.dcode-npm-user-${randomUUID()}.npmrc`);
  const isolatedGlobalConfig = join(tmpdir(), `.dcode-npm-global-${randomUUID()}.npmrc`);
  const isolatedGit = join(tmpdir(), `.dcode-npm-git-${randomUUID()}`);
...
export async function prepareNpmExecutable(
  ...
  try {
    ...
    cwd = mkdtempSync(join(tmpdir(), "dcode-npm-check-"));
