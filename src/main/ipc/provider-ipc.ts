...
import { genericModelConfig, modelConfigWithBinding, mergeProviderHeaders } from "@dcode/agent-runtime";
...
export type ProviderIpcDependencies = {
  ...
  modelsDevCatalog: Pick<ModelsDevCatalog, "ensureLoaded" | "loadLocal" | "getStatus" | "findModel" | "modelsForProvider">;
...
export function registerProviderIpc({
  ...
  // The runtime remote refresh was removed: the bundled release snapshot is the
  // only data source, so this channel reports the capability as unavailable
  // instead of pretending to fetch a newer catalog.
  handle(IPC.invoke.providersRefreshModelCatalog, async () => {
    await modelsDevCatalog.ensureLoaded();
    return { refreshed: false, reason: "unavailable", status: modelsDevCatalog.getStatus() };
  });
