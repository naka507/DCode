...
import { APP_NAME } from "@dcode/shared";

/**
 * The two directories that define an installation, and the development split
 * between them.
 *
 * A packaged dcode and a `pnpm dev` host used to share both: the
 ...
 * `~/.dcode`, where host-core keeps `pi.sqlite` beside the persistence
 ...
 * installation keeps `dcode` and `~/.dcode`, so no upgrade relocates
 * a user's database, secrets, plugins, or renderer-local state, and
 * `DCODE_DATA_DIR` still overrides either profile outright.
 */
...
export const INSTALLATION_DATA_DIR_NAME = ".dcode";

/** Data directory of a development installation, below the user's home. */
export const DEVELOPMENT_DATA_DIR_NAME = ".dcode-dev";

export type DataDirInput = {
  /** `DCODE_DATA_DIR`; an explicit directory wins over either profile. */
 ...
 *
 * `DCODE_DATA_DIR` stays the escape hatch it always was: an explicit
 ...
 * `DCODE_DATA_DIR` at boot. That publication is what keeps the plugin
...
export function desktopDataDir(
  development: boolean = process.env.DCODE_DEV === "1",
): string {
  return resolveDataDir({
    override: process.env.DCODE_DATA_DIR,
    development,
    home: homedir(),
  });
}
