...
export type ApplicationMenuOptions = {
  ...
  /**
   * False when the build has no configured release repository. The "Check for
   * Updates…" item is then omitted from both the macOS app menu and the Help
   * menu, matching the Settings row, the banner and the sidebar chip, all of
   * which hide or go inert in that case.
   */
  updatesConfigured?: boolean;
  /** Runs a configurable app command from the native menu. */
