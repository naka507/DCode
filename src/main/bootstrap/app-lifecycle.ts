...
import { RELEASE_ORIGIN } from "../updater";
...
export function createApplicationLifecycle({
  ...
  function createTray() {
    ...
    // macOS single-click opens its attached menu without focusing/reading a
    // conversation. mouse-enter/move/leave replace the native NSStatusItem with
    // a custom view, which hides the menu-bar extra. Keep hover retry on
    // Windows/Linux only.
