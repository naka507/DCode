...
import { RELEASE_ORIGIN, type AppUpdaterController } from "../updater";
...
export function registerApplicationStartup(deps: StartupDependencies): void {
  ...
  // directory so a `DCODE_DATA_DIR` profile does not share them. Started
  ...
  void app.whenReady().then(async () => {
    ...
    installApplicationMenu({
      ...
      updatesConfigured: RELEASE_ORIGIN !== null,
    ...
    const remoteHostsBoot = createRemoteHostsBoot({
      dataDir,
      encryption: {
        // Electron's safeStorage exposes `isEncryptionAvailable`; this module
      ...
      log: (level, message, data) =>
        logger.app("runtime", level, message, { data: formatRemoteLogData(data) }),
      // The SSH bootstrap downloads the `pi-host` bundle from the release the
      // build is configured against; an unconfigured checkout has no origin and
      // the install fails closed.
      sshBootstrap: { releaseOrigin: RELEASE_ORIGIN },
    });
    ...
    await ensureWindow();
    if (process.env.DCODE_MCP_CONTROL === "1") {
      try {
        state.mcpControl = new McpControlServer({
          ...
          channels: IPC.invoke,
          version: APP_VERSION,
          port: process.env.DCODE_MCP_PORT
            ? Number(process.env.DCODE_MCP_PORT)
            : undefined,
    ...
    // did-finish-load), so the page is up; give the renderer a beat to mount its
    ...
    if (process.env.DCODE_BOOT_PROBE === "1") {
      setTimeout(() => {
        void (async () => {
          try {
            ...
            const probe = await window!.webContents.executeJavaScript(
              `(async () => {
               ...
               const projectRemove = await api.invoke(
                 ...
                 { path: "dcode-boot-probe-unknown-project" },
               );
               return {
                 ok: version?.ok === true,
    ...
    if (process.env.DCODE_SUPERVISION_PROBE === "1") {
