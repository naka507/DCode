...
export async function createWindow({
  ...
  windowState.mainWindow = new BrowserWindow({
    ...
    // own Codex-style window controls (see `.window-controls` in styles/chrome.css).
    // The traffic-light position comes from @dcode/shared so the space
    ...
    webPreferences: {
      ...
      additionalArguments: [`--dcode-locale=${app.getLocale()}`],
    },
  });
  ...
  if (process.platform !== "darwin") {
    ...
    // the renderer mounts, then let the renderer's window controls query it
    // through IPC.
    if (process.env.DCODE_START_MAXIMIZED === "1") window.maximize();
  }
  ...
  window.once("ready-to-show", () => {
    ...
    ensureStableBounds(process.env.DCODE_CAPTURE === "1");
    window.show();
    window.focus();
    ...
    if (process.env.DCODE_CAPTURE === "1") {
      setTimeout(() => {
        void (async () => {
          try {
            ...
            const setPage = async (page: string) => {
              await windowState.mainWindow!.webContents.executeJavaScript(
                `window.__DCODE__?.setPage?.(${JSON.stringify(page)})`,
              );
            };
            const setSettingsTab = async (tab: string) => {
              await windowState.mainWindow!.webContents.executeJavaScript(
                `window.__DCODE__?.setSettingsTab?.(${JSON.stringify(tab)})`,
              );
            };
            ...
            // Wait until the renderer leaves the starting gate.
            for (let i = 0; i < 40; i++) {
            ...
            await windowState.mainWindow!.webContents.executeJavaScript(`
              window.__DCODE__?.setThemeAttr?.("light");
              window.__DCODE__?.setPage?.("chat");
            ...
            try {
              await windowState.mainWindow!.webContents.executeJavaScript(
                `window.__PI_CAPTURE__ = 1; void window.__DCODE__?.ensureVisualFixtures?.()`,
              );
            } catch {
              // fixtures optional
            }
            ...
            if (process.env.DCODE_CAPTURE_STATUS_ONLY === "1") {
              if (process.env.DCODE_CAPTURE_REDUCED_MOTION === "1") {
            ...
            try {
              ...
              if (windowState.host && (existing?.providers?.length ?? 0) === 0) {
                await windowState.host.call("providers.create", {
                  ...
                  secretValue: "sk-capture-fixture-not-a-real-key",
                  apiStyle: "chat_completions",
                });
                await windowState.mainWindow!.webContents.executeJavaScript(
                  `void window.__DCODE__?.refreshProviders?.()`,
                );
                await new Promise((r) => setTimeout(r, 300));
              }
            } catch {
            ...
            await windowState.mainWindow!.webContents.executeJavaScript(
              `window.__DCODE__?.openWorkPanel()`,
            );
            await new Promise((r) => setTimeout(r, 500));
            ...
            const openPanelArtifact = (kind: string, resource?: string) =>
              windowState.mainWindow!.webContents.executeJavaScript(
                `window.__DCODE__?.openWorkPanelArtifact(${JSON.stringify(kind)}, ${JSON.stringify(resource)})`,
              );
            ...
            await windowState.mainWindow!.webContents.executeJavaScript(
              `void window.__DCODE__?.seedReviewChanges?.(4)`,
            );
            await new Promise((r) => setTimeout(r, 500));
            ...
            await windowState.mainWindow!.webContents.executeJavaScript(
              `window.__DCODE__?.setThemeAttr("dark")`,
            );
            await new Promise((r) => setTimeout(r, 350));
            ...
            await new Promise((r) => setTimeout(r, 250));
            await windowState.mainWindow!.webContents.executeJavaScript(
              `void window.__DCODE__?.seedReviewChanges?.(0)`,
            );
            await new Promise((r) => setTimeout(r, 250));
            ...
            await windowState.mainWindow!.webContents.executeJavaScript(
              `void window.__DCODE__?.seedRunRows?.(3)`,
            );
            await new Promise((r) => setTimeout(r, 400));
            await windowState.mainWindow!.webContents.executeJavaScript(`
              (() => {
            ...
            await new Promise((r) => setTimeout(r, 250));
            await windowState.mainWindow!.webContents.executeJavaScript(
              `void window.__DCODE__?.seedDelegationRows?.(0)`,
            );
            await new Promise((r) => setTimeout(r, 250));
            ...
            await windowState.mainWindow!.webContents.executeJavaScript(
              `window.__DCODE__?.openNewWorkPanelTab?.()`,
            );
            await new Promise((r) => setTimeout(r, 350));
            ...
            await openPanelArtifact("file", "src/renderer/App.vue");
            await new Promise((r) => setTimeout(r, 500));
            ...
            // geometry check.
            await windowState.mainWindow!.webContents.executeJavaScript(
              `window.__DCODE__?.setWorkPanelWidth?.(244)`,
            );
            await new Promise((r) => setTimeout(r, 250));
            ...
            await new Promise((r) => setTimeout(r, 250));
            await windowState.mainWindow!.webContents.executeJavaScript(
              `window.__DCODE__?.collapseWorkPanel()`,
            );
            await new Promise((r) => setTimeout(r, 300));
            ...
            // "@" file menu. A controlled textarea needs the native value
            // setter + input event to register the draft.
            ...
            await windowState.mainWindow!.webContents.executeJavaScript(
              `void window.__DCODE__?.seedTranscript?.()`,
            );
            ...
            await windowState.mainWindow!.webContents.executeJavaScript(
              `void window.__DCODE__?.seedTranscript?.(0)`,
            );
            await new Promise((r) => setTimeout(r, 250));
            ...
            const openNotificationFixture = async () => {
              await windowState.mainWindow!.webContents.executeJavaScript(`
                window.__DCODE__?.seedNotifications?.(105);
            ...
            try {
              ...
              await windowState.mainWindow!.webContents.executeJavaScript(`
                document.dispatchEvent(new KeyboardEvent('keydown', { key: 'Escape', bubbles: true }));
                window.__DCODE__?.seedNotifications?.(0);
              `);
            } finally {
              windowState.mainWindow!.setSize(CODEX_BOUNDS.width, CODEX_BOUNDS.height, false);
              windowState.mainWindow!.setMinimumSize(
            ...
            await new Promise((r) => setTimeout(r, 300));
            // Destination + theme captures (robust via __DCODE__ hooks).
            await setTheme("dark");
            await setPage("chat");
            await new Promise((r) => setTimeout(r, 250));
            ...
            await windowState.mainWindow!.webContents.executeJavaScript(
              `window.__DCODE__?.seedPlugins?.(4);
               window.__DCODE__?.seedExtensions?.(3)`,
            );
            ...
            await new Promise((r) => setTimeout(r, 150));
            // The Settings > Agent capability pages (MCP, skills, subagents)
            // moved out of this page; their scenes below are shot from Settings
            // by tab id, and `seedExtensions` has already stood in for the host.
            ...
            await new Promise((r) => setTimeout(r, 200));
            await windowState.mainWindow!.webContents.executeJavaScript(
              `window.__DCODE__?.seedPlugins?.(0);
               window.__DCODE__?.seedExtensions?.(0);
               window.__DCODE__?.seedPluginThemes?.(2)`,
            );
            await setPage("settings");
            ...
            await windowState.mainWindow!.webContents.executeJavaScript(
              `window.__DCODE__?.seedPluginThemes?.(0)`,
            );
            ...
            await clickNav("new-task");
            await new Promise((r) => setTimeout(r, 600));
            // Seeding plugins and extensions earlier raised toasts that outlive
            // the scenes above; clear the viewport so the hero is the only subject.
            const clearToasts = () =>
              windowState.mainWindow!.webContents.executeJavaScript(`
            ...
            const raiseToasts = () =>
              windowState.mainWindow!.webContents.executeJavaScript(`
                window.__DCODE__?.showToast?.("Provider saved", { variant: "success" });
                window.__DCODE__?.showToast?.("Reconnecting to local backend…", { variant: "warning" });
                window.__DCODE__?.showToast?.("Model request failed: 401 Unauthorized", { variant: "error" });
              `);
            await raiseToasts();
            await new Promise((r) => setTimeout(r, 400));
  ...
  if (process.env.ELECTRON_RENDERER_URL) {
    ...
    if (process.env.DCODE_DEVTOOLS === "1") {
