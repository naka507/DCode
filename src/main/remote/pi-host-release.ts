 ...
 */
import { releaseAssetBase, type ReleaseOrigin } from "../repository-url.js";
...
/**
 * Release asset URLs. The release workflow uploads both files to the
 * `v<version>` tag, so the desktop and the bootstrap script derive them without
 * an API call.
 *
 * The origin is an explicit argument: this module stays free of Electron and
 * the filesystem so its URL and checksum rules remain unit-testable, and a
 * checkout with no configured repository has no release host — the caller gets
 * a typed failure instead of a URL that cannot resolve.
 */
export function piHostArtifactUrls(
  ...
  const base = releaseAssetBase(origin, version);
  if (!base) {
    throw new Error("no release repository is configured");
  }
