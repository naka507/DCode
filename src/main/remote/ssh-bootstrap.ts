...
import { type ReleaseOrigin } from "../repository-url.js";
import {
  assertSshArgument,
...
export type SshBootstrapDeps = {
  ...
  version: string;
  /**
   * Release origin the artifact is downloaded from. Omitted and `null` are the
   * same thing: no release host, because this module cannot read the build's
   * `package.json` without pulling in Electron, so the boot layer always passes
   * the resolved origin. With none, the bootstrap fails with
   * `HOST_BOOTSTRAP_FAILED` before it builds a transport — no SSH round-trip.
   */
...
const REMOTE_STEP_MESSAGES: Record<string, string> = {
  ...
  "download-failed": "the remote machine could not download the pi-host bundle from the configured release host",
...
export function createSshBootstrap(deps: SshBootstrapDeps): SshBootstrap {
  ...
  return {
    async bootstrap(request) {
      ...
      // The release origin is a precondition, not a step: with none there is no
      // artifact to fetch, and failing here keeps the refusal before any SSH
      // round-trip (and before `buildTransport`, which would be the first thing
      // to touch the target).
      if (!deps.releaseOrigin) {
        throw fail(
      ...
      try {
        ...
        // The origin was checked above, so it is non-null here; the assertion
        // is for the type checker, not a second guard.
        const urls = piHostArtifactUrls(deps.version, remoteTarget, deps.releaseOrigin ?? null);
