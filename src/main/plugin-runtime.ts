 ...
 * passed to the plugin untouched, so the entry reports the status
 ...
 * yet. Containment is decided by the engine workspace-files checks; this only exists so the
...
export class PluginRuntime {
 ...
 // (e.g. The domain manager's "domain.sync" data bridge).
   ...
   * `DCODE_DATA_DIR` at boot, so this reads the installation's own root
