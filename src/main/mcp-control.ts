...
export type McpControlOperation = {
   ...
   * the dcode_invoke enum) while staying callable by plugins.
   */
  pluginOnly?: boolean;
};
 ...
 * reviewed reads. Secret writes, native pickers, plugin install, window/OS
 * control, and provider/OAuth credential mutations stay out.
...
const CONTROL_OPERATION_SPECS: OperationSpec[] = [
  ...
  spec("providersRefreshModelCatalog", "providers/refreshModelCatalog", "Report the snapshot-only model catalog status.", "read", []),
...
export class McpControlServer {
  ...
  private isAuthorized(request: IncomingMessage): boolean {
    ...
    const token = authorization?.startsWith("Bearer ")
      ...
      : this.headerValue(request, "x-dcode-token");
  ...
  private async handleRequest(request: IncomingMessage, result: ServerResponse): Promise<void> {
    ...
    if (request.method === "OPTIONS") {
      result.writeHead(204, {
        ...
        "Access-Control-Allow-Headers": "Authorization, Content-Type, Mcp-Session-Id, MCP-Protocol-Version, X-Dcode-Token",
